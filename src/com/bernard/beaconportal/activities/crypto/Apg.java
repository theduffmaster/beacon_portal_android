...
public class Apg extends CryptoProvider {
	...
	public static final Uri CONTENT_URI_SECRET_KEY_RING_BY_KEY_ID = Uri
			.parse("content://" + AUTHORITY + "/key_rings/secret/key_id/");
	public static final Uri CONTENT_URI_SECRET_KEY_RING_BY_EMAILS = Uri
			.parse("content://" + AUTHORITY + "/key_rings/secret/emails/");

	public static final Uri CONTENT_URI_PUBLIC_KEY_RING_BY_KEY_ID = Uri
			.parse("content://" + AUTHORITY + "/key_rings/public/key_id/");
	public static final Uri CONTENT_URI_PUBLIC_KEY_RING_BY_EMAILS = Uri
			.parse("content://" + AUTHORITY + "/key_rings/public/emails/");
	...
	// Note: The support package only allows us to use the lower 16 bits of a
	// request code.
		...
		try {
			...
			if (pi.versionCode >= mMinRequiredVersion) {
				...
				Toast.makeText(context,
						R.string.error_apg_version_not_supported,
	...
	public boolean selectSecretKey(Activity activity, PgpData pgpData) {
		...
		try {
			...
			Toast.makeText(activity, R.string.error_activity_not_found,
					Toast.LENGTH_SHORT).show();
			return false;
		}
	}
		...
		if (!pgpData.hasEncryptionKeys()) {
			...
			try {
				Uri contentUri = Uri.withAppendedPath(
						Apg.CONTENT_URI_PUBLIC_KEY_RING_BY_EMAILS, emails);
				...
				Toast.makeText(
						...
						activity.getResources().getString(
								R.string.insufficient_apg_permissions),
	...
	public long[] getSecretKeyIdsFromEmail(Context context, String email) {
		...
		try {
			Uri contentUri = Uri.withAppendedPath(
					Apg.CONTENT_URI_SECRET_KEY_RING_BY_EMAILS, email);
	...
	public long[] getPublicKeyIdsFromEmail(Context context, String email) {
		...
		try {
			Uri contentUri = Uri.withAppendedPath(
					Apg.CONTENT_URI_PUBLIC_KEY_RING_BY_EMAILS, email);
			Cursor c = context.getContentResolver().query(contentUri,
					new String[] { "master_key_id" }, null, null, null);
			if (c != null && c.getCount() > 0) {
				ids = new long[c.getCount()];
	...
	public String getUserId(Context context, long keyId) {
		...
		try {
			Uri contentUri = ContentUris.withAppendedId(
					Apg.CONTENT_URI_SECRET_KEY_RING_BY_KEY_ID, keyId);
			Cursor c = context.getContentResolver().query(contentUri,
		...
		if (userId == null) {
			userId = context
					.getString(R.string.unknown_crypto_signature_user_id);
		}
		return userId;
	}
	...
	public boolean onActivityResult(Activity activity, int requestCode,
		...
		case Apg.SELECT_PUBLIC_KEYS:
			...
			pgpData.setEncryptionKeys(data
					.getLongArrayExtra(Apg.EXTRA_SELECTION));
			((MessageCompose) activity).onEncryptionKeySelectionDone();
			break;
		...
		case Apg.ENCRYPT_MESSAGE:
			...
			pgpData.setEncryptedData(data
					.getStringExtra(Apg.EXTRA_ENCRYPTED_MESSAGE));
			// this was a stupid bug in an earlier version, just gonna leave
			// this in for an APG
			// version or two
	...
	public boolean onDecryptActivityResult(CryptoDecryptCallback callback,
		...
		case Apg.DECRYPT_MESSAGE: {
			...
			pgpData.setSignatureUserId(data
					.getStringExtra(Apg.EXTRA_SIGNATURE_USER_ID));
			pgpData.setSignatureKeyId(data.getLongExtra(
					Apg.EXTRA_SIGNATURE_KEY_ID, 0));
			pgpData.setSignatureSuccess(data.getBooleanExtra(
					Apg.EXTRA_SIGNATURE_SUCCESS, false));
			pgpData.setSignatureUnknown(data.getBooleanExtra(
					Apg.EXTRA_SIGNATURE_UNKNOWN, false));
	...
	public boolean encrypt(Activity activity, String data, PgpData pgpData) {
		...
		intent.putExtra(Apg.EXTRA_ENCRYPTION_KEY_IDS,
				pgpData.getEncryptionKeys());
	...
	public boolean decrypt(Fragment fragment, String data, PgpData pgpData) {
		...
		try {
			...
			Toast.makeText(fragment.getActivity(),
					R.string.error_activity_not_found, Toast.LENGTH_SHORT)
					.show();
			return false;
		}
	}
	...
	public boolean test(Context context) {
		...
		try {
			...
			Uri contentUri = ContentUris.withAppendedId(
					Apg.CONTENT_URI_SECRET_KEY_RING_BY_KEY_ID, 12345);
			Cursor c = context.getContentResolver().query(contentUri,
					new String[] { "user_id" }, null, null, null);
			if (c != null) {
				c.close();

...
public class Address {
		...
		if (parse) {
			...
			if (tokens.length > 0) {
				...
				if (!StringUtils.isNullOrEmpty(name)) {
					/*
					 * Don't use the "personal" argument if "address" is of the
					 * form: James Bond <james.bond@mi6.uk>
					 * 
					 * See issue 2920
					 */
					mPersonal = name;
	...
	/**
	 * Parse a comma separated list of email addresses in human readable format
	 * and return an array of Address objects, RFC-822 encoded.
	 * 
	 * @param addressList
		...
		if (!StringUtils.isNullOrEmpty(addressList)) {
			...
			for (Rfc822Token token : tokens) {
				...
				if (!StringUtils.isNullOrEmpty(address)) {
					addresses.add(new Address(token.getAddress(), token
							.getName(), false));
				}
			}
		}
	...
	public static Address[] parse(String addressList) {
		...
		List<Address> addresses = new ArrayList<Address>();
		try {
			MailboxList parsedList = AddressBuilder.DEFAULT.parseAddressList(
					addressList).flatten();

			for (int i = 0, count = parsedList.size(); i < count; i++) {
				org.apache.james.mime4j.dom.address.Address address = parsedList
						.get(i);
	...
	public boolean equals(Object o) {
		if (o instanceof Address) {
			...
			if (mPersonal != null && other.mPersonal != null
					&& !mPersonal.equals(other.mPersonal)) {
				return false;
			}
	...
	public String toEncodedString() {
		if (!StringUtils.isNullOrEmpty(mPersonal)) {
			return EncoderUtil.encodeAddressDisplayName(mPersonal) + " <"
					+ mAddress + ">";
		} else {
			return mAddress;
		}
	...
	/**
	 * Returns either the personal portion of the Address or the address portion
	 * if the personal is not available.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the name of the contact this email address belongs to if the
	 * {@link Contacts contacts} parameter is not {@code null} and a contact is
	 * found. Otherwise the personal portion of the {@link Address} is returned.
	 * If that isn't available either, the email address is returned.
	 * 
	 * @param contacts
	...
	public CharSequence toFriendly(final Contacts contacts) {
		if (!MAIL.showCorrespondentNames()) {
			...
			// TODO: The results should probably be cached for performance
			// reasons.
	...
	public static Address[] unpack(String addressList) {
		...
		while (pairStartIndex < length) {
			...
			if (addressEndIndex == -1 || addressEndIndex > pairEndIndex) {
				...
				address = addressList
						.substring(pairStartIndex, addressEndIndex);
				personal = addressList.substring(addressEndIndex + 2,
						pairEndIndex);
			}
	...
	/**
	 * Packs an address list into a String that is very quick to read and parse.
	 * Packed lists can be unpacked with unpackAddressList() The packed list is
	 * a ",\u0000" separated list of: address;\u0000personal
	 * 
	 * @param addresses
	 *            Array of addresses to pack.

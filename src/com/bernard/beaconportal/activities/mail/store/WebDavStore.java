...
public class WebDavStore extends Store {
	...
	// These are the ids used from Exchange server to identify the special
	// folders
	 ...
	 *            settings.
	 * 
	 * @return A WebDavStore URI that holds the same information as the
	...
	public static String createUri(ServerSettings server) {
		...
		if (extra != null) {
			...
			String mailboxPath = extra
					.get(WebDavStoreSettings.MAILBOX_PATH_KEY);
		...
		try {
			...
			throw new IllegalArgumentException("Can't create WebDavStore URI",
					e);
		}
	}
	...
	private String mAuthPath; /*
							 * Stores the path off of the server to post data to
	...
	public WebDavStore(Account account) throws MessagingException {
		...
		// The URL typically looks like the following:
		// "https://mail.domain.com/Exchange/alias".
		// The inbox path would look like:
		// "https://mail.domain.com/Exchange/alias/Inbox".
	...
	public List<? extends Folder> getPersonalNamespaces(boolean forceListAll)
		...
		/**
		 * Firstly we get the "special" folders list (inbox, outbox, etc) and
		 * setup the account accordingly
		 */
		HashMap<String, String> headers = new HashMap<String, String>();
		DataSet dataset = new DataSet();
		headers.put("Depth", "0");
		headers.put("Brief", "t");
		dataset = processRequest(this.mUrl, "PROPFIND",
				getSpecialFoldersList(), headers);

		HashMap<String, String> specialFoldersMap = dataset
				.getSpecialFolderToUrl();
		String folderName = getFolderName(specialFoldersMap
				.get(DAV_MAIL_INBOX_FOLDER));
		if (folderName != null) {
			mAccount.setAutoExpandFolderName(folderName);
			mAccount.setInboxFolderName(folderName);
		}

		folderName = getFolderName(specialFoldersMap
				.get(DAV_MAIL_DRAFTS_FOLDER));
		if (folderName != null)
		...
		/*
		 * folderName =
		 * getFolderName(specialFoldersMap.get(DAV_MAIL_OUTBOX_FOLDER)); if
		 * (folderName != null) mAccount.setOutboxFolderName(folderName);
		 */
		...
		dataset = new DataSet();
		headers.put("Brief", "t");
		dataset = processRequest(this.mUrl, "SEARCH", getFolderListXml(),
				headers);
	...
	/**
	 * Creates a folder using the URL passed as parameter (only if it has not
	 * been already created) and adds this to our store folder map.
	 * 
	 * @param folderUrl
	 * @return
	 */
	...
	private String getFolderName(String folderUrl) {
		...
		// folderUrl is in the form
		// http://mail.domain.com/exchange/username/foldername
		...
		if (folderSlash > 0) {
			...
			// Decodes the url-encoded folder name (i.e. "My%20folder" =>
			// "My Folder"
			try {
				folderName = java.net.URLDecoder.decode(fullPathName, "UTF-8");
			} catch (UnsupportedEncodingException uee) {
				/**
				 * If we don't support UTF-8 there's a problem, don't decode it
				 * then
				 */
	...
	private String getSpecialFoldersList() {
		...
		buffer.append("<prop>");
		buffer.append("<").append(DAV_MAIL_INBOX_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
		buffer.append("<").append(DAV_MAIL_DRAFTS_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
		buffer.append("<").append(DAV_MAIL_OUTBOX_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
		buffer.append("<").append(DAV_MAIL_SENT_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
		buffer.append("<").append(DAV_MAIL_TRASH_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
		// This should always be ##DavMailSubmissionURI## for which we already
		// have a constant
		// buffer.append("<sendmsg xmlns=\"urn:schemas:httpmail:\"/>");

		buffer.append("<").append(DAV_MAIL_SPAM_FOLDER)
				.append(" xmlns=\"urn:schemas:httpmail:\"/>");
	...
	private String getFolderListXml() {
		...
		buffer.append(" FROM SCOPE('hierarchical traversal of \"")
				.append(this.mUrl).append("\"')\r\n");
	...
	private String getMessageFlagsXml(String[] uids) throws MessagingException {
		if (uids.length == 0) {
			throw new MessagingException(
					"Attempt to get flags on 0 length array for uids");
		}
	...
	private String getMarkMessagesReadXml(String[] urls, boolean read) {
		...
		buffer.append("  <b:read>").append(read ? "1" : "0")
				.append("</b:read>\r\n");
		buffer.append(" </a:prop>\r\n");
		buffer.append("</a:set>\r\n");
	...
	private String getMoveOrCopyMessagesReadXml(String[] urls, boolean isMove) {
		...
		StringBuilder buffer = new StringBuilder(600);
		buffer.append("<?xml version='1.0' ?>\r\n");
		buffer.append("<a:").append(action)
				.append(" xmlns:a='DAV:' xmlns:b='urn:schemas:httpmail:'>\r\n");
		buffer.append("<a:target>\r\n");
		for (String url : urls) {
			buffer.append(" <a:href>").append(url).append("</a:href>\r\n");
		}
		...
		try {
			if (mAuthentication == AUTH_TYPE_NONE) {
				...
				if (info.requiredAuthType == AUTH_TYPE_BASIC) {
					...
					HttpResponse response = httpClient.executeOverride(request,
							mContext);
					...
					if (statusCode >= 200 && statusCode < 300) {
						...
						throw new MessagingException(
								"Invalid username or password for authentication.");
					} else {
						throw new MessagingException("Error with code "
			...
			Log.e(MAIL.LOG_TAG, "Error during authentication: " + ioe
	...
	private ConnectionInfo doInitialConnection() throws MessagingException {
		...
		try {
			...
			if (info.statusCode == 401) {
				...
				throw new IOException("Error with code " + info.statusCode
						+ " during request processing: "
						+ response.getStatusLine().toString());
			}
			...
			Log.e(MAIL.LOG_TAG, "IOException: " + ioe + "\nTrace: "
	 ...
	 * @throws MessagingException
	 */
	public void doFBA(ConnectionInfo info) throws IOException,
			MessagingException {
		...
		if (info != null) {
			...
			throw new MessagingException(
					"No valid login URL available for form-based authentication.");
		}
		...
		if (!authenticated) {
			...
			// form action.
			String formAction = findFormAction(WebDavHttpClient
					.getUngzippedContent(response.getEntity()));
			if (formAction == null) {
				// If there is no form action, try using our redirect URL from
				// the initial connection.
				if (info != null && info.redirectUrl != null
						&& !info.redirectUrl.equals("")) {
					...
					formAction = findFormAction(WebDavHttpClient
							.getUngzippedContent(response.getEntity()));
				}
			}
			if (formAction != null) {
				try {
					...
					if (formActionUri.isAbsolute()) {
						...
						URI finalUri = new URI(loginUri.getScheme(),
								loginUri.getUserInfo(), loginUri.getHost(),
					...
					Log.e(MAIL.LOG_TAG, "URISyntaxException caught " + e
							+ "\nTrace: " + processException(e));
					throw new MessagingException("URISyntaxException caught", e);
				}
			} else {
				throw new MessagingException(
						"A valid URL for Exchange authentication could not be found.");
			}
		}
	...
	/**
	 * Searches the specified stream for an HTML form and returns the form's
	 * action target.
	 * 
	 * @throws IOException
	 */
	private String findFormAction(InputStream istream) throws IOException {
		...
		// Read line by line until we find something like: <form
		// action="owaauth.dll"...>.
	...
	private boolean testAuthenticationResponse(HttpResponse response)
		...
		if (((statusCode >= 200 && statusCode < 300) || statusCode == 302)
			...
			// We may be authenticated, we need to send a test request to know
			// for sure.
			// Exchange 2007 adds the same cookies whether the username and
			// password were valid or not.
			ConnectionInfo info = doInitialConnection();
			if (info.statusCode >= 200 && info.statusCode < 300) {
				authenticated = true;
			} else if (info.statusCode == 302) {
				// If we are successfully authenticated, Exchange will try to
				// redirect us to our OWA inbox.
				...
				try {
					...
					if (redirectPath.equalsIgnoreCase(thisPath)) {
						...
						if (found != -1) {
							...
							redirectPath = redirectPath.replace("/owa/",
									replace);
							if (redirectPath.equalsIgnoreCase(thisPath)) {
								authenticated = true;
							}
	...
	public WebDavHttpClient getHttpClient() throws MessagingException {
		if (mHttpClient == null) {
			...
			SchemeRegistry reg = mHttpClient.getConnectionManager()
					.getSchemeRegistry();
			try {
				Scheme s = new Scheme("https", new WebDavSocketFactory(mHost,
						443), 443);
				reg.register(s);
				...
				Log.e(MAIL.LOG_TAG, "KeyManagementException in getHttpClient: "
						+ kme);
				throw new MessagingException(
						"KeyManagementException in getHttpClient: " + kme);
			}
		}
		return mHttpClient;
	}

	private InputStream sendRequest(String url, String method,
			StringEntity messageBody, HashMap<String, String> headers,
			boolean tryAuth) throws MessagingException {
		...
		try {
			...
			if (mAuthentication == AUTH_TYPE_NONE) {
				if (!tryAuth || !authenticate()) {
					throw new MessagingException(
							"Unable to authenticate in sendRequest().");
				}
			} else if (mAuthentication == AUTH_TYPE_BASIC) {
			...
			if (statusCode == 401) {
				throw new MessagingException(
						"Invalid username or password for Basic authentication.");
				...
				if (tryAuth && mAuthentication == AUTH_TYPE_FORM_BASED) {
					...
					throw new MessagingException(
							"Authentication failure in sendRequest().");
				}
			} else if (statusCode < 200 || statusCode >= 300) {
				throw new IOException("Error with code " + statusCode
						+ " during request processing: "
						+ response.getStatusLine().toString());
			}
			...
			Log.e(MAIL.LOG_TAG, "UnsupportedEncodingException: " + uee
	 ...
	 * There are two signatures to support calls that don't require parsing of
	 * the response.
	 */
	private DataSet processRequest(String url, String method,
			String messageBody, HashMap<String, String> headers)
			throws MessagingException {
	...
	private DataSet processRequest(String url, String method,
		...
		try {
			...
			InputStream istream = sendRequest(url, method, messageEntity,
					headers, true);
			if (istream != null && needsParsing) {
				try {
					...
					Log.e(MAIL.LOG_TAG,
							"ParserConfigurationException in processRequest() "
			...
			Log.e(MAIL.LOG_TAG, "IOException: " + ioe + "\nTrace: "
					+ processException(ioe));
			throw new MessagingException("IOException in processRequest() ",
					ioe);
		}
	...
	/**
	 * Returns a string of the stacktrace for a Throwable to allow for easy
	 * inline printing of errors.
	 */
	...
	public void sendMessages(Message[] messages) throws MessagingException {
		WebDavFolder tmpFolder = (WebDavStore.WebDavFolder) getFolder(mAccount
				.getDraftsFolderName());
		try {
	...
	class WebDavFolder extends Folder {
			...
			try {
				...
				for (int i = 0, count = urlParts.length; i < count; i++) {
					if (i != 0) {
						url = url
								+ "/"
								+ java.net.URLEncoder.encode(urlParts[i],
										"UTF-8");
					} else {
				...
				Log.e(MAIL.LOG_TAG,
						"UnsupportedEncodingException URLEncoding folder name, skipping encoded");
				encodedName = name;
			}
		...
		@Override
		public void delete(Message[] msgs, String trashFolderName)
				throws MessagingException {
			moveOrCopyMessages(msgs, trashFolderName, true);
		}

		private void moveOrCopyMessages(Message[] messages, String folderName,
				boolean isMove) throws MessagingException {
			...
			WebDavFolder destFolder = (WebDavFolder) store
					.getFolder(folderName);
			headers.put("Destination", destFolder.mFolderUrl);
			headers.put("Brief", "t");
			headers.put("If-Match", "*");
			String action = (isMove ? "BMOVE" : "BCOPY");
			Log.i(MAIL.LOG_TAG, "Moving " + messages.length + " messages to "
					+ destFolder.mFolderUrl);
		...
		private int getMessageCount(boolean read) throws MessagingException {
			...
			headers.put("Brief", "t");
			DataSet dataset = processRequest(this.mFolderUrl, "SEARCH",
					messageBody, headers);
			if (dataset != null) {
				messageCount = dataset.getMessageCount();
			}
			if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_WEBDAV) {
				Log.v(MAIL.LOG_TAG, "Counted messages and webdav returned: "
						+ messageCount);
			}
		...
		public void fetch(Message[] messages, FetchProfile fp,
			...
			if (fp.contains(FetchProfile.Item.BODY_SANE)) {
				if (mAccount.getMaximumAutoDownloadMessageSize() > 0) {
					fetchMessages(messages, listener,
		...
		/**
		 * Fetches the full messages or up to lines lines and passes them to the
		 * message parser.
		 */
		private void fetchMessages(Message[] messages,
				MessageRetrievalListener listener, int lines)
				throws MessagingException {
			WebDavHttpClient httpclient;
			httpclient = getHttpClient();

			/**
			 * We can't hand off to processRequest() since we need the stream to
			 * parse.
			 */
			for (int i = 0, count = messages.length; i < count; i++) {
				...
				if (!(messages[i] instanceof WebDavMessage)) {
					throw new MessagingException(
							"WebDavStore fetch called with non-WebDavMessage");
				}
				...
				/**
				 * If fetch is called outside of the initial list (ie, a locally
				 * stored message), it may not have a URL associated. Verify and
				 * fix that
				 */
				if (wdMessage.getUrl().equals("")) {
					wdMessage.setUrl(getMessageUrls(
							new String[] { wdMessage.getUid() }).get(
							wdMessage.getUid()));
					Log.i(MAIL.LOG_TAG,
							"Fetching messages with UID = '"
									+ wdMessage.getUid() + "', URL = '"
									+ wdMessage.getUrl() + "'");
					if (wdMessage.getUrl().equals("")) {
						throw new MessagingException(
								"Unable to get URL for message");
					}
				}
				...
				try {
					...
					if (statusCode < 200 || statusCode > 300) {
						throw new IOException("Error during with code "
								+ statusCode + " during fetch: "
								+ response.getStatusLine().toString());
					}
					...
					if (entity != null) {
						...
						try {
							istream = WebDavHttpClient
									.getUngzippedContent(entity);

							if (lines != -1) {
								reader = new BufferedReader(
										new InputStreamReader(istream), 8192);
					...
					Log.e(MAIL.LOG_TAG, "URISyntaxException caught " + use
							+ "\nTrace: " + processException(use));
					throw new MessagingException("URISyntaxException caught",
							use);
				} catch (IOException ioe) {
					Log.e(MAIL.LOG_TAG,
							"Non-success response code loading message, response code was "
									...
									+ ioe.getMessage() + "\nTrace: "
									+ processException(ioe));
					throw new MessagingException("Failure code " + statusCode,
							ioe);
				}
		...
		/**
		 * Fetches and sets the message flags for the supplied messages. The
		 * idea is to have this be recursive so that we do a series of medium
		 * calls instead of one large massive call or a large number of smaller
		 * calls.
		 */
		private void fetchFlags(Message[] startMessages,
				MessageRetrievalListener listener) throws MessagingException {
			HashMap<String, String> headers = new HashMap<String, String>();
			String messageBody = "";
			...
			if (startMessages == null || startMessages.length == 0) {
				return;
			}
			...
			for (int i = 0, count = messages.length; i < count; i++) {
				...
				try {
					wdMessage.setFlagInternal(Flag.SEEN,
							uidToReadStatus.get(wdMessage.getUid()));
		 ...
		 * The idea is to have this be recursive so that we do a series of
		 * medium calls instead of one large massive call or a large number of
		 * smaller calls. Call it a happy balance
		 */
		private void fetchEnvelope(Message[] startMessages,
				MessageRetrievalListener listener) throws MessagingException {
			HashMap<String, String> headers = new HashMap<String, String>();
			String messageBody = "";
			String[] uids;
			...
			Map<String, ParsedMessageEnvelope> envelopes = dataset
					.getMessageEnvelopes();
			...
			for (int i = messages.length - 1; i >= 0; i--) {
				...
				ParsedMessageEnvelope envelope = envelopes.get(wdMessage
						.getUid());
				if (envelope != null) {
					wdMessage.setNewHeaders(envelope);
					wdMessage.setFlagInternal(Flag.SEEN,
							envelope.getReadStatus());
				} else {
					Log.e(MAIL.LOG_TAG,
							"Asked to get metadata for a non-existent message: "
									+ wdMessage.getUid());
				}
		...
		private void markServerMessagesRead(String[] uids, boolean read)
				throws MessagingException {
			String messageBody = "";
			HashMap<String, String> headers = new HashMap<String, String>();
			HashMap<String, String> uidToUrl = getMessageUrls(uids);
			...
			processRequest(this.mFolderUrl, "BPROPPATCH", messageBody, headers,
					false);
		}
		...
		private void deleteServerMessages(String[] uids)
			...
			for (String uid : uids) {
				...
				/**
				 * If the destination is the same as the origin, assume delete
				 * forever
				 */
		...
		private String generateDeleteUrl(String startUrl) {
			...
			String finalUrl = WebDavStore.this.mUrl + "Deleted%20Items/"
					+ filename;
		...
		public Message[] appendWebDavMessages(Message[] messages)
				throws MessagingException {
			...
			for (Message message : messages) {
				...
				try {
					...
					messageURL += URLEncoder.encode(message.getUid() + ":"
							+ System.currentTimeMillis() + ".eml", "UTF-8");

					Log.i(MAIL.LOG_TAG, "Uploading message as " + messageURL);
					...
					if (statusCode < 200 || statusCode > 300) {
						throw new IOException(
								"Error with status code "
										...
										+ response.getStatusLine().toString()
										+ " for message " + messageURL);
					}
					WebDavMessage retMessage = new WebDavMessage(
							message.getUid(), this);
	...
	class WebDavMessage extends MimeMessage {
		...
		public void setNewHeaders(ParsedMessageEnvelope envelope)
				throws MessagingException {
			String[] headers = envelope.getHeaderList();
			HashMap<String, String> messageHeaders = envelope
					.getMessageHeaders();
		...
		public void delete(String trashFolderName) throws MessagingException {
			...
			Log.i(MAIL.LOG_TAG, "Deleting message by moving to "
					+ trashFolderName);
			wdFolder.moveMessages(new Message[] { this }, wdFolder.getStore()
	 ...
	 * Only provides accessor methods to the stored data. All processing should
	 * be done elsewhere. This is done rather than having multiple hashmaps
	 * associating UIDs to values
	 */
	public static class ParsedMessageEnvelope {
		/**
		 * Holds the mappings from the name returned from Exchange to the MIME
		 * format header name
		 */
	...
	/**
	 * Dataset for all XML parses. Data is stored in a single format inside the
	 * class and is formatted appropriately depending on the accessor calls
	 * made.
	 */
	public class DataSet {
		...
		public void finish() {
			...
			if (uid != null && mTempData != null) {
				 ...
				 * These requests should only have a depth of one for the
				 * response so it will never get stomped over.
				 */
			}
		...
		public HashMap<String, Boolean> getUidToRead() {
			...
			for (String uid : mData.keySet()) {
				...
				if (readStatus != null && !readStatus.equals("")) {
					...
					// We don't actually want to have null values in our
					// hashmap,
				...
				if (data != null) {
					for (Map.Entry<String, String> entry : data.entrySet()) {
						...
						if (header.equals("read")) {
							...
							/**
							 * Exchange doesn't give us rfc822 dates like it
							 * claims. The date is in the format:
							 * yyyy-MM-dd'T'HH:mm:ss.SSS<Single digit
							 * representation of timezone, so far, all instances
							 * are Z>
							 */
							String date = entry.getValue();
							date = date.substring(0, date.length() - 1);

							DateFormat dfInput = new SimpleDateFormat(
									"yyyy-MM-dd'T'HH:mm:ss.SSS", Locale.US);
							DateFormat dfOutput = new SimpleDateFormat(
									"EEE, d MMM yy HH:mm:ss Z", Locale.US);
							String tempDate = "";
							...
							try {
								...
								Log.e(MAIL.LOG_TAG, "Error parsing date: " + pe
	...
	public class HttpGeneric extends HttpEntityEnclosingRequestBase {
		...
		public HttpGeneric(final String uri) {
			...
			if (MAIL.DEBUG) {
				Log.v(MAIL.LOG_TAG, "Starting uri = '" + uri + "'");
			}
			...
			try {
				...
				Log.e(MAIL.LOG_TAG,
						"UnsupportedEncodingException caught in HttpGeneric(String uri): "
								+ uee + "\nTrace: " + processException(uee));
			} catch (IllegalArgumentException iae) {
				Log.e(MAIL.LOG_TAG,
						"IllegalArgumentException caught in HttpGeneric(String uri): "
								+ iae + "\nTrace: " + processException(iae));
			}
	...
	public static class WebDavHttpClient extends DefaultHttpClient {
		...
		public static void modifyRequestToAcceptGzipResponse(HttpRequest request) {
			Log.i(MAIL.LOG_TAG, "Requesting gzipped data");
		...
		public static InputStream getUngzippedContent(HttpEntity entity)
			...
			if (contentEncoding.contains("gzip")) {
				Log.i(MAIL.LOG_TAG, "Response is gzipped");
				responseStream = new GZIPInputStream(responseStream);
			}
			return responseStream;
		}

		public HttpResponse executeOverride(HttpUriRequest request,

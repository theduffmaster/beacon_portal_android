...
public class LockableDatabase {
	...
	public static interface DbCallback<T> {
		 ...
		 */
		T doDbWork(SQLiteDatabase db) throws WrappedException,
	 ...
	 *            Never <code>null</code
	 */
	public LockableDatabase(final Application application, final String uUid,
	 ...
	 *             If storage can't be locked because it is not available.
	 */
	protected void lockWrite(final String providerId)
			throws UnavailableStorageException {
		mWriteLock.lock();
		try {
	 ...
	 * @throws UnavailableStorageException
	 */
	public <T> T execute(final boolean transactional,
			final DbCallback<T> callback) throws UnavailableStorageException {
		lockRead();
		...
		try {
			...
			try {
				...
				if (doTransaction) {
					...
					// not doing endTransaction in the same 'finally' block of
					// unlockRead() because endTransaction() may throw an
					// exception
					mDb.endTransaction();
					if (debug) {
						Log.v(MAIL.LOG_TAG,
	...
	public void switchProvider(final String newProviderId)
		...
		try {
			...
			try {
				...
				File oldDatabase = storageManager.getDatabase(uUid,
						oldProviderId);
				...
				Utility.moveRecursive(oldDatabase,
						storageManager.getDatabase(uUid, newProviderId));
				// move all attachment files
				Utility.moveRecursive(storageManager.getAttachmentDirectory(
						uUid, oldProviderId), storageManager
						.getAttachmentDirectory(uUid, newProviderId));
	 ...
	 * @throws UnavailableStorageException
	 */
	protected void openOrCreateDataspace(final Application application)
			throws UnavailableStorageException {
		...
		try {
			...
			try {
				if (StorageManager.InternalStorageProvider.ID
						.equals(mStorageProviderId)) {
					// internal storage
					mDb = application.openOrCreateDatabase(
	...
	protected File prepareStorage(final String providerId)
		...
		if (databaseParentDir.isFile()) {
			// should be safe to unconditionally delete clashing file: user is
			// not supposed to mess with our directory
			databaseParentDir.delete();
		}
		if (!databaseParentDir.exists()) {
			if (!databaseParentDir.mkdirs()) {
				// Android seems to be unmounting the storage...
				throw new UnavailableStorageException("Unable to access: "
						+ databaseParentDir);
			}
		...
		attachmentDir = storageManager.getAttachmentDirectory(uUid, providerId);

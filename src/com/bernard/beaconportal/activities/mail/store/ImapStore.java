...
import com.bernard.beaconportal.activities.mail.store.ImapResponseParser.ImapList;
...
public class ImapStore extends Store {
	...
	public static ImapStoreSettings decodeUri(String uri) {
		...
		/*
		 * Currently available schemes are: imap imap+tls+ imap+ssl+
		 * 
		 * The following are obsolete schemes that may be found in pre-existing
		 * settings from earlier versions or that may be found when imported. We
		 * continue to recognize them and re-map them appropriately: imap+tls
		 * imap+ssl
		 */
		...
		if (imapUri.getUserInfo() != null) {
			try {
				...
				if (userinfo.endsWith(":")) {
					// Password is empty. This can only happen after an account
					// was imported.
				...
				throw new IllegalArgumentException(
						"Couldn't urldecode username or password.", enc);
			}
		}
	 ...
	 * 
	 * @param server
	 *            The {@link ServerSettings} object that holds the server
	 *            settings.
	 * 
	 * @return An ImapStore URI that holds the same information as the
	 *         {@code server} parameter.
	 * 
	...
	public static String createUri(ServerSettings server) {
		...
		try {
			...
			if (extra != null) {
				...
				String pathPrefix = (autoDetectNamespace) ? null : extra
						.get(ImapStoreSettings.PATH_PREFIX_KEY);
				path = "/" + (autoDetectNamespace ? "1" : "0") + "|"
						+ ((pathPrefix == null) ? "" : pathPrefix);
			} else {
				path = "/1|";
			}
			return new URI(scheme, userInfo, server.host, server.port, path,
					null, null).toString();
	...
	public static class ImapStoreSettings extends ServerSettings {
		...
		public Map<String, String> getExtra() {
			...
			extra.put(AUTODETECT_NAMESPACE_KEY,
					Boolean.valueOf(autoDetectNamespace).toString());
	...
	private static final SimpleDateFormat RFC3501_DATE = new SimpleDateFormat(
	...
	public ImapStore(Account account) throws MessagingException {
		...
		// configured
		mPathPrefix = (settings.autoDetectNamespace) ? null
				: settings.pathPrefix;

		mModifiedUtf7Charset = new CharsetProvider()
				.charsetForName("X-RFC-3501");
	}
	...
	private String getCombinedPrefix() {
		if (mCombinedPrefix == null) {
			if (mPathPrefix != null) {
				...
				String tmpDelim = (mPathDelimeter != null ? mPathDelimeter
						.trim() : "");
	...
	public List<? extends Folder> getPersonalNamespaces(boolean forceListAll)
		...
		try {
			...
			if (forceListAll || !mAccount.subscribedFoldersOnly()) {
				...
				List<? extends Folder> subscribedFolders = listFolders(
						connection, true);
	...
	private List<? extends Folder> listFolders(ImapConnection connection,
		...
		List<ImapResponse> responses = connection.executeSimpleCommand(String
				.format("%s \"\" %s", commandResponse,
		...
		for (ImapResponse response : responses) {
			if (ImapResponseParser.equalsIgnoreCase(response.get(0),
				...
				if (response.size() > 4
						|| !(response.getObject(3) instanceof String)) {
					Log.w(MAIL.LOG_TAG, "Skipping incorrectly parsed "
							+ commandResponse + " reply: " + response);
					continue;
				}
				...
				if (folder.equalsIgnoreCase(mAccount.getInboxFolderName())) {
					...
					if (prefixLength > 0) {
						...
						if (!decodedFolderName
								.equalsIgnoreCase(getCombinedPrefix() + folder)) {
							includeFolder = false;
						}
					}
				}
	...
	/**
	 * Attempt to auto-configure folders by attributes if the server advertises
	 * that capability.
	 * 
	 * The parsing here is essentially the same as
	 * {@link #listFolders(com.bernard.beaconportal.activities.mail.store.ImapStore.ImapConnection, boolean)}
	 ...
	 *             uh oh!
	 */
	private void autoconfigureFolders(final ImapConnection connection)
		...
		if (connection.capabilities.contains("XLIST")) {
			if (MAIL.DEBUG)
				Log.d(MAIL.LOG_TAG, "Folder auto-configuration: Using XLIST.");
			commandResponse = "XLIST";
		} else if (connection.capabilities.contains("SPECIAL-USE")) {
			if (MAIL.DEBUG)
				Log.d(MAIL.LOG_TAG,
						"Folder auto-configuration: Using RFC6154/SPECIAL-USE.");
			commandResponse = "LIST";
			commandOptions = " (SPECIAL-USE)";
		} else {
			if (MAIL.DEBUG)
				Log.d(MAIL.LOG_TAG,
						"No detected folder auto-configuration methods.");
			return;
		}

		final List<ImapResponse> responses = connection
				.executeSimpleCommand(String.format("%s%s \"\" %s",
						commandResponse, commandOptions,
						encodeString(getCombinedPrefix() + "*")));
	...
	public void checkSettings() throws MessagingException {
		try {
			...
			throw new MessagingException(
					MAIL.app.getString(R.string.error_unable_to_connect), ioe);
		}
	}
...
/**
	...
	private String encodeFolderName(String name) {
		try {
			 ...
			 */
			throw new RuntimeException("Unable to encode folder name: " + name,
					uee);
		}
	}
	...
	private String decodeFolderName(String name)
		...
		try {
			...
			// invalid encoding.
			CharsetDecoder decoder = mModifiedUtf7Charset.newDecoder()
					.onMalformedInput(CodingErrorAction.REPORT);
			CharBuffer cb = decoder.decode(ByteBuffer.wrap(name
					.getBytes("US-ASCII")));
			return cb.toString();
		} catch (UnsupportedEncodingException uee) {
			/*
			 ...
			 * US-ASCII doesn't exist we're totally screwed.
			 */
			throw new RuntimeException("Unable to decode folder name: " + name,
					uee);
		}
	}
	...
	class ImapFolder extends Folder {
		...
		public String getPrefixedName() throws MessagingException {
			...
			if (!mAccount.getInboxFolderName().equalsIgnoreCase(mName)) {
				...
				try {
					...
					throw new MessagingException("Unable to get IMAP prefix",
							ioe);
				} finally {
					if (mConnection == null) {
						releaseConnection(connection);
		...
		protected List<ImapResponse> executeSimpleCommand(String command)
			...
			return handleUntaggedResponses(mConnection
					.executeSimpleCommand(command));
		}

		protected List<ImapResponse> executeSimpleCommand(String command,
				boolean sensitve, UntaggedHandler untaggedHandler)
				throws MessagingException, IOException {
			return handleUntaggedResponses(mConnection.executeSimpleCommand(
					command, sensitve, untaggedHandler));
		}
		...
		public List<ImapResponse> internalOpen(int mode)
				throws MessagingException {
			...
			try {
				...
				String command = String.format("%s %s",
						mode == OPEN_MODE_RW ? "SELECT" : "EXAMINE",
				...
				for (ImapResponse response : responses) {
					if (response.size() >= 2) {
						...
						ImapList flags = bracketed
								.getKeyedList("PERMANENTFLAGS");
						if (flags != null) {
							// parse: * OK [PERMANENTFLAGS (\Answered \Flagged
							// \Deleted
							// \Seen \Draft NonJunk $label1 \*)] Flags
							// permitted.
							parseFlags(flags);
						} else {
		...
		public void close() {
			...
			synchronized (this) {
				// If we are mid-search and we get a close request, we gotta
		 ...
		 * instances are used. It is assumed that all UIDs represent valid
		 ...
		 */
		@Override
		public Map<String, String> copyMessages(Message[] messages,
			...
			try {
				String remoteDestName = encodeString(encodeFolderName(iFolder
						.getPrefixedName()));

				// TODO: Try to copy/move the messages first and only create the
				// folder if the
				// operation fails. This will save a roundtrip if the folder
				// already exists.
				...
				// TODO: Split this into multiple commands if the command
				// exceeds a certain length.
				List<ImapResponse> responses = executeSimpleCommand(String
						.format("UID COPY %s %s", Utility.combine(uids, ','),
								remoteDestName));
				...
				if (response.size() > 1) {
					 ...
					 * 
					 * COPYUID is followed by UIDVALIDITY, the set of UIDs of
					 * copied messages from the source folder and the set of
					 * corresponding UIDs assigned to them in the destination
					 * folder.
					 * 
					 * We can use the new UIDs included in this response to
					 * update our records.
					 */
					...
					if (responseList instanceof ImapList) {
						...
						if (copyList.size() >= 4
							...
							List<String> srcUids = ImapUtility
									.getImapSequenceValues(copyList
											.getString(2));
							...
							if (srcUids != null && destUids != null) {
								if (srcUids.size() == destUids.size()) {
									...
									while (srcUidsIterator.hasNext()
										...
										String destUid = destUidsIterator
												.next();
		...
		@Override
		public Map<String, String> moveMessages(Message[] messages,
				Folder folder) throws MessagingException {
			if (messages.length == 0)
				return null;
		...
		@Override
		public void delete(Message[] messages, String trashFolderName)
				throws MessagingException {
			if (messages.length == 0)
				return;
			...
			if (trashFolderName == null
				...
				ImapFolder remoteTrashFolder = (ImapFolder) getStore()
						.getFolder(trashFolderName);
				String remoteTrashName = encodeString(encodeFolderName(remoteTrashFolder
						.getPrefixedName()));
		...
		private int getRemoteMessageCount(String criteria)
			...
			try {
				...
				List<ImapResponse> responses = executeSimpleCommand(String
						.format(Locale.US, "SEARCH %d:* %s", start, criteria));
				for (ImapResponse response : responses) {
					if (ImapResponseParser.equalsIgnoreCase(response.get(0),
							"SEARCH")) {
		...
		protected long getHighestUid() {
			try {
				...
				Message[] messages = search(searcher, null).toArray(
						EMPTY_MESSAGE_ARRAY);
				if (messages.length > 0) {
					return Long.parseLong(messages[0].getUid());
				}
			} catch (Exception e) {
				Log.e(MAIL.LOG_TAG, "Unable to find highest UID in folder "
						+ getName(), e);
			}
			return -1L;
		...
		protected Message[] getMessages(final int start, final int end,
			...
			ImapSearcher searcher = new ImapSearcher() {
				...
				public List<ImapResponse> search() throws IOException,
					...
					return executeSimpleCommand(String.format(Locale.US,
							"UID SEARCH %d:%d%s%s", start, end,
							dateSearchString, includeDeleted ? ""
									: " NOT DELETED"));
				}
			};
		...
		protected Message[] getMessages(final List<Long> mesgSeqs,
		...
		private List<Message> search(ImapSearcher searcher,
				MessageRetrievalListener listener) throws MessagingException {
			...
			try {
				...
				// By doing it in decreasing order, we ensure newest messages
		...
		public void fetch(Message[] messages, FetchProfile fp,
			...
			if (fp.contains(FetchProfile.Item.ENVELOPE)) {
				...
				fetchFields
						.add("BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc "
								...
								+ MAIL.IDENTITY_HEADER + ")]");
			}
			if (fp.contains(FetchProfile.Item.STRUCTURE)) {
				fetchFields.add("BODYSTRUCTURE");
			}
			if (fp.contains(FetchProfile.Item.BODY_SANE)) {
				// If the user wants to download unlimited-size messages, don't
				// go only for the truncated body
				if (mAccount.getMaximumAutoDownloadMessageSize() > 0) {
					fetchFields.add(String.format(Locale.US,
							"BODY.PEEK[]<0.%d>",
			...
			for (int windowStart = 0; windowStart < messages.length; windowStart += (FETCH_WINDOW_SIZE)) {
				List<String> uidWindow = uids.subList(windowStart, Math.min(
						(windowStart + FETCH_WINDOW_SIZE), messages.length));

				try {
					mConnection.sendCommand(String
							.format("UID FETCH %s (%s)", Utility.combine(
									uidWindow.toArray(new String[uidWindow
											.size()]), ','), Utility.combine(
									fetchFields.toArray(new String[fetchFields
											.size()]), ' ')), false);
		...
		public void fetchPart(Message message, Part part,
			...
			try {
				...
				ImapResponseParser.IImapResponseCallback callback = new FetchPartCallback(
						part);
		...
		private Object handleFetchResponse(ImapMessage message,
				ImapList fetchList) throws MessagingException {
			Object result = null;
				...
				if (flags != null) {
					for (int i = 0, count = flags.size(); i < count; i++) {
						...
						if (flag.equalsIgnoreCase("\\Deleted")) {
							...
							/*
							 * a message contains FORWARDED FLAG -> so we can
							 * also create them
							 */
							mPermanentFlagsIndex.add(Flag.FORWARDED);
						}
			...
			if (fetchList.containsKey("BODYSTRUCTURE")) {
				...
				if (bs != null) {
					try {
						...
						if (MAIL.DEBUG)
							Log.d(MAIL.LOG_TAG, "Error handling message for "
									+ getLogId(), e);
		 ...
		 * @param responses
		 */
		protected List<ImapResponse> handleUntaggedResponses(
					...
					if (bracketed.size() > 1) {
						...
						if (keyObj instanceof String) {
							...
							if ("UIDNEXT".equalsIgnoreCase(key)) {
								...
								if (MAIL.DEBUG)
									Log.d(MAIL.LOG_TAG, "Got UidNext = "
											+ uidNext + " for " + getLogId());
							}
						}
					}
		...
		protected void handleUntaggedResponse(ImapResponse response) {
			if (response.mTag == null && response.size() > 1) {
				if (ImapResponseParser.equalsIgnoreCase(response.get(1),
						"EXISTS")) {
					mMessageCount = response.getNumber(0);
					if (MAIL.DEBUG)
						Log.d(MAIL.LOG_TAG, "Got untagged EXISTS with value "
								+ mMessageCount + " for " + getLogId());
				}
				handlePossibleUidNext(response);

				if (ImapResponseParser.equalsIgnoreCase(response.get(1),
						"EXPUNGE") && mMessageCount > 0) {
					mMessageCount--;
					if (MAIL.DEBUG)
						Log.d(MAIL.LOG_TAG,
								"Got untagged EXPUNGE with mMessageCount "
										+ mMessageCount + " for " + getLogId());
				}
				// if (response.size() > 1) {
				// Object bracketedObj = response.get(1);
				...
				// }
				//
				// Log.w(MAIL.LOG_TAG, "ALERT: " + sb.toString() + " for " +
				// getLogId());
				// }
				// }
				// }
			...
			if (bs.get(0) instanceof ImapList) {
				...
				if (bodyDisposition != null && !bodyDisposition.isEmpty()) {
					...
					if ((bodyDisposition.size() > 1)
						...
						ImapList bodyDispositionParams = bodyDisposition
								.getList(1);
						/*
				...
				if (MimeUtility.getHeaderParameter(
						contentDisposition.toString(), "size") == null) {
				...
				part.setHeader(MimeHeader.HEADER_ANDROID_ATTACHMENT_STORE_DATA,
						id);
			}
		 ...
		 * 
		 * <p>
		 * This implementation also determines the new UIDs of the given
		 * messages on the IMAP server and changes the messages' UIDs to the new
		 * server UIDs.
		 * </p>
		 * 
		 * @param messages
				...
				for (Message message : messages) {
					...
					do {
						...
						if (response.mCommandContinuationRequested) {
							EOLConvertingOutputStream eolOut = new EOLConvertingOutputStream(
									mConnection.mOut);
					...
					if (response.size() > 1) {
						 ...
						 * 
						 * We can use the UID included in this response to
						 * update our records.
						 */
						Object responseList = response.get(1);
					 ...
					 * UIDPLUS or does not implement the APPENDUID response
					 * code.
					 */
					String newUid = getUidFromMessageId(message);
					if (MAIL.DEBUG) {
						Log.d(MAIL.LOG_TAG, "Got UID " + newUid
		...
		public String getUidFromMessageId(Message message)
			...
			try {
				...
				throw new MessagingException(
						"Could not find UID for message based on Message-ID",
						ioe);
			}
		}
		...
		public String getNewPushState(String oldPushStateS, Message message) {
			try {
				...
				Log.e(MAIL.LOG_TAG, "Exception while updated push state for "
						+ getLogId(), e);
				return null;
			}
		}
		...
		private MessagingException ioExceptionHandler(
				ImapConnection connection, IOException ioe) {
			Log.e(MAIL.LOG_TAG, "IOException for " + getLogId(), ioe);
			if (connection != null) {
				connection.close();
			}
			close();
		 ...
		 */
		@Override
		public List<Message> search(final String queryString,
			...
			try {
				...
				mInSearch = true;
				// don't pass listener--we don't want to add messages until
	...
	public static class ImapConnection {
		...
		private List<ImapResponse> receiveCapabilities(
				List<ImapResponse> responses) {
			for (ImapResponse response : responses) {
				...
				if (!response.isEmpty()
					...
					for (Object thisPart : response) {
						if (thisPart instanceof ImapList) {
							...
							if (ImapResponseParser.equalsIgnoreCase(
									thisList.get(0), CAPABILITY_CAPABILITY)) {
				...
				if (capabilityList != null
						...
						&& ImapResponseParser.equalsIgnoreCase(
								capabilityList.get(0), CAPABILITY_CAPABILITY)) {
					if (MAIL.DEBUG) {
						Log.d(MAIL.LOG_TAG, "Saving " + capabilityList.size()
					...
					for (Object capability : capabilityList) {
						if (capability instanceof String) {
							// if (MAIL.DEBUG)
							// {
							// Log.v(MAIL.LOG_TAG, "Saving capability '" +
		...
		public void open() throws IOException, MessagingException {
			...
			try {
				...
				Log.w(MAIL.LOG_TAG, "Could not set DNS ttl to 0 for "
						+ getLogId(), e);
			}
			...
			try {
				...
				Log.w(MAIL.LOG_TAG, "Could not set DNS negative ttl to 0 for "
						+ getLogId(), e);
			}

			try {
				ConnectionSecurity connectionSecurity = mSettings
						.getConnectionSecurity();

				// Try all IPv4 and IPv6 addresses of the host
				InetAddress[] addresses = InetAddress.getAllByName(mSettings
						.getHost());
				for (int i = 0; i < addresses.length; i++) {
					try {
						if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_IMAP) {
							Log.d(MAIL.LOG_TAG,
									"Connecting to " + mSettings.getHost()
											+ " as " + addresses[i]);
						}
						...
						// Successfully connected to the server; don't try any
						// other addresses
						break;
					} catch (SocketException e) {
						if (i < (addresses.length - 1)) {
							// There are still other addresses for that host to
							// try
							continue;
						}
				...
				if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_IMAP)
					Log.v(MAIL.LOG_TAG, getLogId() + "<<<" + nullResponse);
				...
				if (mSettings.getConnectionSecurity() == ConnectionSecurity.STARTTLS_REQUIRED) {
					...
					if (hasCapability("STARTTLS")) {
						...
						mOut = new BufferedOutputStream(
								mSocket.getOutputStream(), 1024);
						// Per RFC 2595 (3.1): Once TLS has been started,
						// reissue CAPABILITY command
						if (MAIL.DEBUG)
							Log.i(MAIL.LOG_TAG,
				...
				authSuccess = true;
				if (MAIL.DEBUG) {
					Log.d(MAIL.LOG_TAG, CAPABILITY_COMPRESS_DEFLATE + " = "
							+ hasCapability(CAPABILITY_COMPRESS_DEFLATE));
				}
				if (hasCapability(CAPABILITY_COMPRESS_DEFLATE)) {
					ConnectivityManager connectivityManager = (ConnectivityManager) MAIL.app
					...
					if (netInfo != null) {
						...
						if (MAIL.DEBUG)
							Log.d(MAIL.LOG_TAG, "On network type " + type);
						useCompression = mSettings.useCompression(type);

					}
					if (MAIL.DEBUG)
						Log.d(MAIL.LOG_TAG, "useCompression " + useCompression);
					if (useCompression) {
						try {
							...
							mIn = new PeekableInputStream(
									new BufferedInputStream(zInputStream, 1024));
							mParser = new ImapResponseParser(mIn);
							...
							if (MAIL.DEBUG) {
								Log.i(MAIL.LOG_TAG, "Compression enabled for "
										+ getLogId());
							}
						} catch (Exception e) {
							Log.e(MAIL.LOG_TAG,
				...
				if (MAIL.DEBUG)
					Log.d(MAIL.LOG_TAG, "NAMESPACE = "
							+ hasCapability(CAPABILITY_NAMESPACE)
							...
							if (ImapResponseParser.equalsIgnoreCase(
								...
								if (personalNamespaces != null
									...
									if (firstNamespace != null
										...
										mSettings.setPathPrefix(bracketed
												.getString(0));
										mSettings.setPathDelimeter(bracketed
												.getString(1));
				...
				if (mSettings.getPathDelimeter() == null) {
					try {
						...
						for (ImapResponse response : nameResponses) {
							if (ImapResponseParser.equalsIgnoreCase(
									response.get(0), "LIST")) {
								mSettings.setPathDelimeter(response
										.getString(2));
								mSettings.setCombinedPrefix(null);
								if (MAIL.DEBUG)
									Log.d(MAIL.LOG_TAG, "Got path delimeter '"
		...
		protected void login() throws IOException, MessagingException {
			 ...
			 * string literals would be better, but some servers are broken and
			 * don't parse them correctly.)
			 */
			...
			try {
				receiveCapabilities(executeSimpleCommand(String.format(
						"LOGIN \"%s\" \"%s\"", username, password), true));
			...
			do {
				...
				if (response.mTag != null) {
					if (response.mTag.equalsIgnoreCase(tag)) {
						...
						Log.w(MAIL.LOG_TAG, "After sending tag " + tag
		...
		protected ArrayList<ImapResponse> readStatusResponse(String tag,
			...
			do {
				...
				if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_IMAP)
					Log.v(MAIL.LOG_TAG, getLogId() + "<<<" + response);

				if (response.mTag != null
						&& !response.mTag.equalsIgnoreCase(tag)) {
					Log.w(MAIL.LOG_TAG, "After sending tag " + tag
							+ ", got tag response from previous command "
							+ response + " for " + getLogId());
			...
			if (response.size() < 1
					|| !ImapResponseParser.equalsIgnoreCase(response.get(0),
							"OK")) {
				throw new ImapException("Command: " + commandToLog
		...
		protected boolean isIdleCapable() {
			if (MAIL.DEBUG)
				Log.v(MAIL.LOG_TAG, "Connection " + getLogId() + " has "
		...
		public ImapResponse readResponse(
				ImapResponseParser.IImapResponseCallback callback)
				throws IOException {
			try {
		...
		public void sendContinuation(String continuation) throws IOException {
			...
			if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_IMAP)
				Log.v(MAIL.LOG_TAG, getLogId() + ">>> " + continuation);
			...
			try {
				...
				if (MAIL.DEBUG && MAIL.DEBUG_PROTOCOL_IMAP) {
					if (sensitive && !MAIL.DEBUG_SENSITIVE) {
						...
						Log.v(MAIL.LOG_TAG, getLogId() + ">>> " + commandToSend);
					}
				}
		...
		public List<ImapResponse> executeSimpleCommand(String command,
			...
			// if (MAIL.DEBUG)
			// Log.v(MAIL.LOG_TAG, "Sending IMAP command " + commandToLog +
			// " on connection " + getLogId());

			String tag = sendCommand(command, sensitive);
			// if (MAIL.DEBUG)
			// Log.v(MAIL.LOG_TAG, "Sent IMAP command " + commandToLog +
	...
	public class ImapFolderPusher extends ImapFolder implements UntaggedHandler {
		...
		public ImapFolderPusher(ImapStore store, String name,
				PushReceiver nReceiver) {
			super(store, name);
			receiver = nReceiver;
			TracingPowerManager pm = TracingPowerManager
					.getPowerManager(receiver.getContext());
			wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
					"ImapFolderPusher " + store.getAccount().getDescription()
							+ ":" + getName());
		...
		public void refresh() throws IOException, MessagingException {
			if (idling.get()) {
				wakeLock.acquire(MAIL.PUSH_WAKE_LOCK_TIMEOUT);
				sendDone();
			}
		}
		...
		public void start() {
			Runnable runner = new Runnable() {
				...
				public void run() {
					wakeLock.acquire(MAIL.PUSH_WAKE_LOCK_TIMEOUT);
					if (MAIL.DEBUG)
						Log.i(MAIL.LOG_TAG, "Pusher starting for " + getLogId());
					...
					while (!stop.get()) {
						try {
							long oldUidNext = -1L;
							try {
								String pushStateS = receiver
										.getPushState(getName());
								ImapPushState pushState = ImapPushState
										.parse(pushStateS);
								oldUidNext = pushState.uidNext;
								if (MAIL.DEBUG)
									Log.i(MAIL.LOG_TAG, "Got oldUidNext "
											+ oldUidNext + " for " + getLogId());
							} catch (Exception e) {
								Log.e(MAIL.LOG_TAG,
										"Unable to get oldUidNext for "
												+ getLogId(), e);
							}

							/*
							 * This makes sure 'oldUidNext' is never smaller
							 * than 'UIDNEXT' from the last loop iteration. This
							 * way we avoid looping endlessly causing the
							 * battery to drain.
							 * 
							 * See issue 4907
							 */
							...
							if (newUidNext == -1) {
								...
								if (highestUid != -1L) {
									if (MAIL.DEBUG)
										Log.d(MAIL.LOG_TAG, "highest UID = "
												+ highestUid);
							...
							if (newUidNext > startUid) {
								...
								if (MAIL.DEBUG)
									Log.i(MAIL.LOG_TAG, "Needs sync from uid "
											+ startUid + " to " + newUidNext
											+ " for " + getLogId());
								List<Message> messages = new ArrayList<Message>();
								for (long uid = startUid; uid < newUidNext; uid++) {
									ImapMessage message = new ImapMessage(""
											+ uid, ImapFolderPusher.this);
									messages.add(message);
								}
								if (!messages.isEmpty()) {
									...
									if (MAIL.DEBUG)
										Log.i(MAIL.LOG_TAG,
												"Processing "
														...
														+ getLogId());
									untaggedResponses = new ArrayList<ImapResponse>(
											storedUntaggedResponses);
									storedUntaggedResponses.clear();
									processUntaggedResponses(untaggedResponses);
								}
								...
								if (MAIL.DEBUG)
									Log.i(MAIL.LOG_TAG, "About to IDLE for "
											+ getLogId());
								...
								conn.setReadTimeout((getAccount()
										.getIdleRefreshMinutes() * 60 * 1000)
										+ IDLE_READ_TIMEOUT_INCREMENT);
								untaggedResponses = executeSimpleCommand(
										COMMAND_IDLE, false,
							...
							try {
								...
								Log.e(MAIL.LOG_TAG,
										"Got exception while closing for exception for "
												+ getLogId(), me);
							}
							if (stop.get()) {
								Log.i(MAIL.LOG_TAG,
										"Got exception while idling, but stop is set for "
												+ getLogId());
							} else {
								receiver.pushError("Push error for "
										+ getName(), e);
								Log.e(MAIL.LOG_TAG,
										"Got exception while idling for "
												+ getLogId(), e);
								...
								if (idleFailureCount.incrementAndGet() > IDLE_FAILURE_COUNT_LIMIT) {
									Log.e(MAIL.LOG_TAG, "Disabling pusher for "
											+ getLogId() + " after "
											+ idleFailureCount.get()
											+ " consecutive errors");
									receiver.pushError("Push disabled for "
											+ getName() + " after "
											+ idleFailureCount.get()
					...
					try {
						if (MAIL.DEBUG)
							Log.i(MAIL.LOG_TAG, "Pusher for " + getLogId()
									+ " is exiting");
						close();
					} catch (Exception me) {
						Log.e(MAIL.LOG_TAG, "Got exception while closing for "
								+ getLogId(), me);
					} finally {
						wakeLock.release();
					}
		...
		protected void handleUntaggedResponse(ImapResponse response) {
			if (response.mTag == null && response.size() > 1) {
				...
				if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")
						|| ImapResponseParser.equalsIgnoreCase(responseType,
								"EXPUNGE")
						|| ImapResponseParser.equalsIgnoreCase(responseType,
								"EXISTS")) {
					if (MAIL.DEBUG)
						Log.d(MAIL.LOG_TAG, "Storing response " + response
		...
		protected void processUntaggedResponses(List<ImapResponse> responses)
			...
			for (ImapResponse response : responses) {
				oldMessageCount += processUntaggedResponse(oldMessageCount,
						response, flagSyncMsgSeqs, removeMsgUids);
			}
			if (!skipSync) {
			...
			if (MAIL.DEBUG)
				Log.d(MAIL.LOG_TAG, "UIDs for messages needing flag sync are "
		...
		private void syncMessages(int end, boolean newArrivals)
				throws MessagingException {
			long oldUidNext = -1L;
			try {
			...
			if (messageArray != null && messageArray.length > 0) {
				...
				if (newUid >= startUid) {
					...
					if (MAIL.DEBUG)
						Log.i(MAIL.LOG_TAG, "Needs sync from uid " + startUid
								+ " to " + newUid + " for " + getLogId());
					List<Message> messages = new ArrayList<Message>();
					for (long uid = startUid; uid <= newUid; uid++) {
						ImapMessage message = new ImapMessage(
								Long.toString(uid), ImapFolderPusher.this);
						messages.add(message);
					}
					if (!messages.isEmpty()) {
						pushMessages(messages, true);
		...
		private void syncMessages(List<Long> flagSyncMsgSeqs) {
			try {
				...
				receiver.pushError(
						"Exception while processing Push untagged responses", e);
			}
		}
		...
		private void removeMessages(List<String> removeUids) {
			...
			try {
				Message[] existingMessages = getMessagesFromUids(removeUids,
						true, null);
				for (Message existingMessage : existingMessages) {
					needsPoll.set(true);
					msgSeqUidMap.clear();
					String existingUid = existingMessage.getUid();
					Log.w(MAIL.LOG_TAG, "Message with UID " + existingUid
				...
				for (String uid : removeUids) {
					...
					try {
						...
						Log.e(MAIL.LOG_TAG,
								"Unable to set DELETED flag on message "
										+ message.getUid());
					}
					messages.add(message);
				}
				...
				Log.e(MAIL.LOG_TAG, "Cannot remove EXPUNGEd messages", e);
			}

		}

		protected int processUntaggedResponse(long oldMessageCount,
				ImapResponse response, List<Long> flagSyncMsgSeqs,
			...
			if (response.mTag == null && response.size() > 1) {
				try {
					Object responseType = response.get(1);
					if (ImapResponseParser.equalsIgnoreCase(responseType,
							"FETCH")) {
						Log.i(MAIL.LOG_TAG, "Got FETCH " + response);
						long msgSeq = response.getLong(0);
					...
					if (ImapResponseParser.equalsIgnoreCase(responseType,
						...
						if (MAIL.DEBUG)
							Log.d(MAIL.LOG_TAG,
									"Got untagged EXPUNGE for msgseq " + msgSeq
											+ " for " + getLogId());
						...
						List<Long> msgSeqs = new ArrayList<Long>(
								msgSeqUidMap.keySet());
					...
					Log.e(MAIL.LOG_TAG, "Could not handle untagged FETCH for "
							+ getLogId(), e);
				}
			}
		...
		public void stop() {
			...
			if (conn != null) {
				if (MAIL.DEBUG)
					Log.v(MAIL.LOG_TAG,
							"Closing mConnection to stop pushing for "
									+ getLogId());
				conn.close();
			} else {
				Log.w(MAIL.LOG_TAG,
		...
		public void handleAsyncUntaggedResponse(ImapResponse response) {
			if (MAIL.DEBUG)
				Log.v(MAIL.LOG_TAG, "Got async response: " + response);

			if (stop.get()) {
				if (MAIL.DEBUG)
					Log.d(MAIL.LOG_TAG, "Got async untagged response: "
							+ response + ", but stop is set for " + getLogId());

				try {
					sendDone();
				} catch (Exception e) {
					Log.e(MAIL.LOG_TAG, "Exception while sending DONE for "
							+ getLogId(), e);
				}
			} else {
				if (response.mTag == null) {
					if (response.size() > 1) {
						...
						if (ImapResponseParser.equalsIgnoreCase(responseType,
							...
							if (MAIL.DEBUG)
								Log.d(MAIL.LOG_TAG,
										"Got useful async untagged response: "
												+ response + " for "
												+ getLogId());
							...
							try {
								...
								Log.e(MAIL.LOG_TAG,
										"Exception while sending DONE for "
												+ getLogId(), e);
							}
						}
					} else if (response.mCommandContinuationRequested) {
						if (MAIL.DEBUG)
							Log.d(MAIL.LOG_TAG, "Idling " + getLogId());
	...
	public class ImapPusher implements Pusher {
		...
		public void start(List<String> folderNames) {
			...
			synchronized (folderPushers) {
				...
				for (String folderName : folderNames) {
					...
					if (pusher == null) {
						pusher = new ImapFolderPusher(mStore, folderName,
								mReceiver);
		...
		public void stop() {
			if (MAIL.DEBUG)
				Log.i(MAIL.LOG_TAG, "Requested stop of IMAP pusher");
			...
			synchronized (folderPushers) {
				for (ImapFolderPusher folderPusher : folderPushers.values()) {
					try {
						if (MAIL.DEBUG)
							Log.i(MAIL.LOG_TAG,
									"Requesting stop of IMAP folderPusher "
											+ folderPusher.getName());
						folderPusher.stop();
					} catch (Exception e) {
						Log.e(MAIL.LOG_TAG, "Got exception while stopping "
								+ folderPusher.getName(), e);
					}
				}
				folderPushers.clear();
	...
	private static class FetchBodyCallback implements
			ImapResponseParser.IImapResponseCallback {
		...
		public Object foundLiteral(ImapResponse response,
			...
			if (response.mTag == null
					&& ImapResponseParser.equalsIgnoreCase(response.get(1),
							"FETCH")) {
	...
	private static class FetchPartCallback implements
			ImapResponseParser.IImapResponseCallback {
		private Part mPart;

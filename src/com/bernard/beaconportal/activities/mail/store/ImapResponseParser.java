...
public class ImapResponseParser {
	...
	private static final SimpleDateFormat badDateTimeFormat = new SimpleDateFormat(
			"dd MMM yyyy HH:mm:ss Z", Locale.US);
	private static final SimpleDateFormat badDateTimeFormat2 = new SimpleDateFormat(
			"E, dd MMM yyyy HH:mm:ss Z", Locale.US);
	private static final SimpleDateFormat badDateTimeFormat3 = new SimpleDateFormat(
		...
		try {
			...
			if (mException != null) {
				throw new RuntimeException(
						"readResponse(): Exception in callback method",
						mException);
			}
	 ...
	 * 
	 * <p>
	 * Responses "OK", "PREAUTH", "BYE", "NO", "BAD", and continuation request
	 * responses can contain {@code resp-text} tokens. We parse the
	 * {@code resp-text-code} part as tokens and read the rest as sequence of
	 * characters to avoid the parser interpreting things like "{123}" as start
	 * of a literal.
	 * </p>
	 * <p>
	 * Example:
	 ...
	 * 
	 * @param parent
	 *            The {@link ImapResponse} instance that holds the parsed tokens
	 *            of the response.
	 * 
	 * @throws IOException
	 ...
	 * for NIL, QUOTED, NUMBER, ATOM. Object - for LITERAL. ImapList - for
	 * PARENTHESIZED LIST. Can contain any of the above elements including List.
	 * 
		...
		while (true) {
			...
			if (ch == -1) {
				throw new IOException("parseAtom(): end of stream reached");
					...
					|| ch == '[' || ch == ']' ||
					// docs claim that flags are \ atom but atom isn't supposed
					// to
					// contain
	...
	private int expect(char ch) throws IOException {
		...
		if ((d = mIn.read()) != ch) {
			throw new IOException(String.format(
					"Expected %04x (%c) but got %04x (%c)", (int) ch, ch, d,
					(char) d));
		}
		return d;
	}
	...
	public static class ImapList extends ArrayList<Object> {
		...
		private Date getDate(String value) throws MessagingException {
			try {
				...
				throw new MessagingException("Unable to parse IMAP datetime '"
						+ value + "' ", pe);
			}
		}
		...
		public int getKeyIndex(Object key) {
			...
			throw new IllegalArgumentException(
					"getKeyIndex() only works for keys that are in the collection.");
		}
	 ...
	 * 
	 * <p>
	 * Tagged responses will have a non-null tag. Untagged responses will have a
	 * null tag. The object will contain all of the available tokens at the time
	...
	public class ImapResponse extends ImapList {
		...
		public String toString() {
			return "#" + (mCommandContinuationRequested ? "+" : mTag) + "# "
					+ super.toString();
		}
	}

	public boolean isStatusResponse(String symbol) {
		return symbol.equalsIgnoreCase("OK") || symbol.equalsIgnoreCase("NO")
	...
	public interface IImapResponseCallback {
		/**
		 * Callback method that is called by the parser when a literal string is
		 * found in an IMAP response.
		 * 
		 * @param response
		 *            ImapResponse object with the fields that have been parsed
		 *            up until now (excluding the literal string).
		 * @param literal
		 *            FixedLengthInputStream that can be used to access the
		 *            literal string.
		 * 
		 ...
		 * 
		 * @throws IOException
		 *             passed-through if thrown by FixedLengthInputStream
		 * @throws Exception
		 *             if something goes wrong. Parsing will be resumed and the
		 *             exception will be thrown after the complete IMAP response
		 *             has been parsed.
		 */

...
public class MimeUtility {
	...
	public static final String[][] MIME_TYPE_BY_EXTENSION_MAP = new String[][] {
			...
			{ "xof", "x-world/x-vrml" }, { "xop", "application/xop+xml" },
			...
			{ "zip", "application/zip" }, { "zir", "application/vnd.zul" },
	...
	private static final String[][] MIME_TYPE_REPLACEMENT_MAP = new String[][] {
			{ "image/jpg", "image/jpeg" }, { "image/pjpeg", "image/jpeg" }, // see
																			// issue
																			// 1712
			{ "application/x-zip-compressed", "application/zip" } // see issue
																	// 3791
	};
	...
	private static final String[][] CHARSET_FALLBACK_MAP = new String[][] {
			...
			{ "koi8-u", "koi8-r" }, { "iso-2022-jp-[\\d]+", "iso-2022-jp" },
	...
	public static String getHeaderParameter(String header, String name) {
		...
		for (String part : parts) {
			if (part.trim().toLowerCase(Locale.US)
					.startsWith(name.toLowerCase(Locale.US))) {
				String[] partParts = part.split("=", 2);
				if (partParts.length == 2) {
					String parameter = partParts[1].trim();
					int len = parameter.length();
					if (len >= 2 && parameter.startsWith("\"")
							&& parameter.endsWith("\"")) {
	 ...
	 * conversion that needed to be done. Note, this <b>does not</b> return a
	 * text representation of HTML.
	 * 
	 * @param part
	 *            The part containing a body
	 * @return a String containing the converted text in the body, or null if
	 *         there was no text or an error during conversion.
	 */
	public static String getTextFromPart(Part part) {
		try {
			if ((part != null) && (part.getBody() != null)) {
				...
				if ((mimeType != null)
						&& MimeUtility.mimeTypeMatches(mimeType, "text/*")) {
					/*
						...
						try {
							...
							try {
								if (in instanceof BinaryTempFileBodyInputStream) {
									/*
									 * If this is a
									 * BinaryTempFileBodyInputStream, calling
									 * close() will delete the file. But we
									 * can't let that happen because the file
									 * needs to be opened again by the code a
									 * few lines down.
									 */
									((BinaryTempFileBodyInputStream) in)
					...
					try {
						...
						// Replace the body with a TextBody that already
						...
						try {
							/*
							 * This time we don't care if it's a
							 * BinaryTempFileBodyInputStream. We replaced the
							 * body with a TextBody instance and hence don't
	...
	public static Body decodeBody(InputStream in,
		...
		if (contentTransferEncoding != null) {
			contentTransferEncoding = MimeUtility.getHeaderParameter(
					contentTransferEncoding, null);
			if (MimeUtil.ENC_QUOTED_PRINTABLE
					.equalsIgnoreCase(contentTransferEncoding)) {
				in = new QuotedPrintableInputStream(in);
			} else if (MimeUtil.ENC_BASE64
					.equalsIgnoreCase(contentTransferEncoding)) {
	...
	/**
	 * Class representing textual parts of a message that aren't marked as
	 * attachments.
	 * 
	 ...
	 * 
	 * <p>
	 * This is used to extract basic header information when the message
	 * contents are displayed inline.
	 * </p>
	 */
	 ...
	 * 
	 * <p>
	 * Only relevant {@code text/plain} and {@code text/html} children are
	 * stored in this container class.
	 * </p>
	 */
	...
	/**
	 * Store viewable text of a message as plain text and HTML, and the parts
	...
	public static class ViewableContainer {
		...
		/**
		 * The parts of the message considered attachments (everything not
		 * viewable).
		 */
	...
	/**
	 * Extract the viewable textual parts of a message and return the rest as
	 * attachments.
	 * 
	 * @param context
	 *            A {@link Context} instance that will be used to get localized
	 *            strings.
	 * @param message
	 *            The message to extract the text and attachments from.
	 * 
	 * @return A {@link ViewableContainer} instance containing the textual parts
	 *         of the message as plain text and HTML, and a list of message
	 *         parts considered attachments.
	 * 
	 * @throws MessagingException
	 *             In case of an error.
	 */
	public static ViewableContainer extractTextAndAttachments(Context context,
		...
		try {
			...
			for (Viewable viewable : viewables) {
				if (viewable instanceof Textual) {
					// This is either a text/plain or text/html part. Fill the
					// variables 'text' and
					// 'html', converting between plain text and HTML as
					// necessary.
					...
					/*
					 * We made sure at least one of text/plain or text/html is
					 * present when creating the Alternative object. If one part
					 * is not present we convert the other one to make sure
					 * 'text' and 'html' always contain the same text.
					 */
					List<Viewable> textAlternative = alternative.getText()
							.isEmpty() ? alternative.getHtml() : alternative
							.getText();
					List<Viewable> htmlAlternative = alternative.getHtml()
							.isEmpty() ? alternative.getText() : alternative
							.getHtml();
	 ...
	 * @param attachments
	 *            A list that will receive the parts that are considered
	 *            attachments.
	 * 
	 ...
	 *             In case of an error.
	 */
	public static List<Viewable> getViewables(Part part, List<Part> attachments)
			throws MessagingException {
		...
		if (body instanceof Multipart) {
			...
			if (part.getMimeType().equalsIgnoreCase("multipart/alternative")) {
				/*
				 * For multipart/alternative parts we try to find a text/plain
				 * and a text/html child. Everything else we find is put into
				 * 'attachments'.
				 */
				List<Viewable> text = findTextPart(multipart, true);

				Set<Part> knownTextParts = getParts(text);
				List<Viewable> html = findHtmlPart(multipart, knownTextParts,
						attachments, true);
				...
				// For all other multipart parts we recurse to grab all viewable
				// children.
			...
			/*
			 * We only care about message/rfc822 parts whose Content-Disposition
			 * header has a value other than "attachment".
			 */
			...
			// Recurse to grab all viewable parts and attachments from that
			// message.
	 ...
	 * @param directChild
	 *            If {@code true}, this method will return after the first
	 *            {@code text/plain} was found.
	 * 
	 ...
	 *             In case of an error.
	 */
	private static List<Viewable> findTextPart(Multipart multipart,
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (body instanceof Multipart) {
				...
				/*
				 * Recurse to find text parts. Since this is a multipart that is
				 * a child of a multipart/alternative we don't want to stop
				 * after the first text/plain part we find. This will allow to
				 * get all text parts for constructions like this:
				 * 
				 * 1. multipart/alternative 1.1. multipart/mixed 1.1.1.
				 * text/plain 1.1.2. text/plain 1.2. text/html
				 */
				List<Viewable> textViewables = findTextPart(innerMultipart,
						false);
				...
				if (!textViewables.isEmpty()) {
					...
					&& part.getMimeType().equalsIgnoreCase("text/plain")) {
				Text text = new Text(part);
				viewables.add(text);
				if (directChild) {
					break;
	 ...
	 * 
	 * <p>
	 * Every part that is not a {@code text/html} we want to display, we add to
	 * 'attachments'.
	 * </p>
	 * 
	 * @param multipart
	 ...
	 *            A set of {@code text/plain} parts that shouldn't be added to
	 *            'attachments'.
	 * @param attachments
	 *            A list that will receive the parts that are considered
	 *            attachments.
	 * @param directChild
	 *            If {@code true}, this method will add all {@code text/html}
	 *            parts except the first found to 'attachments'.
	 * 
	 * @return A list of {@link Text} viewables.
	 * 
	 * @throws MessagingException
	 *             In case of an error.
	 */
	private static List<Viewable> findHtmlPart(Multipart multipart,
			Set<Part> knownTextParts, List<Part> attachments,
			boolean directChild) throws MessagingException {
		List<Viewable> viewables = new ArrayList<Viewable>();
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (body instanceof Multipart) {
				...
				if (directChild && partFound) {
					// We already found our text/html part. Now we're only
					// looking for attachments.
					findAttachments(innerMultipart, knownTextParts, attachments);
				} else {
					/*
					 * Recurse to find HTML parts. Since this is a multipart
					 * that is a child of a multipart/alternative we don't want
					 * to stop after the first text/html part we find. This will
					 * allow to get all text parts for constructions like this:
					 * 
					 * 1. multipart/alternative 1.1. text/plain 1.2.
					 * multipart/mixed 1.2.1. text/html 1.2.2. text/html 1.3.
					 * image/jpeg
					 */
					List<Viewable> htmlViewables = findHtmlPart(innerMultipart,
							knownTextParts, attachments, false);
				...
				// Only add this part as attachment if it's not a viewable
	 ...
	 * 
	 * @param viewables
	 *            A list of {@link Viewable}s containing references to the
	 *            message parts to include in the set.
	 * 
	...
	/**
	 * Traverse the MIME tree and add everything that's not a known text part to
	 * 'attachments'.
	 * 
	 * @param multipart
	 *            The {@link Multipart} to start from.
	 * @param knownTextParts
	 *            A set of known text parts we don't want to end up in
	 *            'attachments'.
	 * @param attachments
	 *            A list that will receive the parts that are considered
	 *            attachments.
	 */
	private static void findAttachments(Multipart multipart,
			Set<Part> knownTextParts, List<Part> attachments) {
		int childCount = multipart.getCount();
		for (int i = 0; i < childCount; i++) {
			Part part = multipart.getBodyPart(i);
	...
	/**
	 * Extract important header values from a message to display inline (plain
	 * text version).
	 * 
	 * @param context
	 *            A {@link Context} instance that will be used to get localized
	 *            strings.
	 * @param text
	 *            The {@link StringBuilder} that will receive the (plain text)
	 *            output.
	 * @param message
	 ...
	 *             In case of an error.
	 */
	private static void addMessageHeaderText(Context context,
			StringBuilder text, Message message) throws MessagingException {
		// From: <sender>
		Address[] from = message.getFrom();
		if (from != null && from.length > 0) {
			text.append(context
					.getString(R.string.message_compose_quote_header_from));
			text.append(' ');
		...
		if (to != null && to.length > 0) {
			text.append(context
					.getString(R.string.message_compose_quote_header_to));
			text.append(' ');
		...
		if (cc != null && cc.length > 0) {
			text.append(context
					.getString(R.string.message_compose_quote_header_cc));
			text.append(' ');
		...
		if (date != null) {
			text.append(context
					.getString(R.string.message_compose_quote_header_send_date));
			text.append(' ');
		...
		text.append(context
				.getString(R.string.message_compose_quote_header_subject));
		text.append(' ');
		if (subject == null) {
	...
	/**
	 * Extract important header values from a message to display inline (HTML
	 * version).
	 * 
	 * @param context
	 *            A {@link Context} instance that will be used to get localized
	 ...
	 *             In case of an error.
	 */
	private static void addMessageHeaderHtml(Context context,
			StringBuilder html, Message message) throws MessagingException {
		...
		if (from != null && from.length > 0) {
			addTableRow(
					html,
					context.getString(R.string.message_compose_quote_header_from),
		...
		if (to != null && to.length > 0) {
			addTableRow(
					html,
					context.getString(R.string.message_compose_quote_header_to),
					Address.toString(to));
		}
		...
		if (cc != null && cc.length > 0) {
			addTableRow(
					html,
					context.getString(R.string.message_compose_quote_header_cc),
					Address.toString(cc));
		}
		...
		if (date != null) {
			addTableRow(
					html,
					context.getString(R.string.message_compose_quote_header_send_date),
					date.toString());
		}
		...
		addTableRow(
				html,
				context.getString(R.string.message_compose_quote_header_subject),
				(subject == null) ? context
						.getString(R.string.general_no_subject) : subject);
	 ...
	 */
	private static void addTableRow(StringBuilder html, String header,
			String value) {
	...
	/**
	 * Use the contents of a {@link Viewable} to create the plain text to be
	 * displayed.
	 * 
	 * <p>
	 * This will use {@link HtmlConverter#htmlToText(String)} to convert HTML
	 * parts to plain text if necessary.
	 * </p>
	 * 
	 * @param viewable
	 *            The viewable part to build the text from.
	 * @param prependDivider
	 *            {@code true}, if the text divider should be inserted as first
	 *            element. {@code false}, otherwise.
	 * 
	 * @return The contents of the supplied viewable instance as plain text.
	 */
	private static StringBuilder buildText(Viewable viewable,
			boolean prependDivider) {
		StringBuilder text = new StringBuilder();
		if (viewable instanceof Textual) {
			...
			// That's odd - an Alternative as child of an Alternative; go ahead
			// and try to use the
			// text/plain child; fall-back to the text/html part.
			Alternative alternative = (Alternative) viewable;

			List<Viewable> textAlternative = alternative.getText().isEmpty() ? alternative
					.getHtml() : alternative.getText();
	 ...
	 * @param part
	 *            The message part that will follow after the divider. This is
	 *            used to extract the part's name.
	 * @param prependDivider
	 *            {@code true}, if the divider should be appended. {@code false}
	 *            , otherwise.
	 */
	private static void addTextDivider(StringBuilder text, Part part,
			boolean prependDivider) {
		if (prependDivider) {
			...
			if (len > 0) {
				if (len > TEXT_DIVIDER_LENGTH - FILENAME_PREFIX_LENGTH
						- FILENAME_SUFFIX_LENGTH) {
					filename = filename.substring(0, TEXT_DIVIDER_LENGTH
							- FILENAME_PREFIX_LENGTH - FILENAME_SUFFIX_LENGTH
							- 3)
							+ "...";
				}
				...
				text.append(TEXT_DIVIDER.substring(0, TEXT_DIVIDER_LENGTH
						- FILENAME_PREFIX_LENGTH - filename.length()
						- FILENAME_SUFFIX_LENGTH));
			} else {
	...
	/**
	 * Use the contents of a {@link Viewable} to create the HTML to be
	 * displayed.
	 * 
	 * <p>
	 * This will use {@link HtmlConverter#textToHtml(String)} to convert plain
	 * text parts to HTML if necessary.
	 * </p>
	 * 
	 * @param viewable
	 *            The viewable part to build the HTML from.
	 * @param prependDivider
	 *            {@code true}, if the HTML divider should be inserted as first
	 *            element. {@code false}, otherwise.
	 * 
	 * @return The contents of the supplied viewable instance as HTML.
	 */
	private static StringBuilder buildHtml(Viewable viewable,
			boolean prependDivider) {
		StringBuilder html = new StringBuilder();
		if (viewable instanceof Textual) {
			Part part = ((Textual) viewable).getPart();
			...
			List<Viewable> htmlAlternative = alternative.getHtml().isEmpty() ? alternative
					.getText() : alternative.getHtml();
	 ...
	 *            , otherwise.
	 */
	private static void addHtmlDivider(StringBuilder html, Part part,
			boolean prependDivider) {
		if (prependDivider) {
			String filename = getPartName(part);
	 ...
	 * 
	 * @return The (file)name of the part if available. An empty string,
	 *         otherwise.
	 */
	private static String getPartName(Part part) {
		try {
			...
			if (disposition != null) {
				String name = MimeUtility.getHeaderParameter(disposition,
						"filename");
	 ...
	 * 
	 * @return The value of the {@code Content-Disposition} header if available.
	 *         {@code null}, otherwise.
	 */
	...
	public static Boolean isPartTextualBody(Part part)
		...
		if (disposition != null) {
			...
			dispositionFilename = MimeUtility.getHeaderParameter(disposition,
					"filename");
		}
	...
	public static String getMimeTypeByExtension(String filename) {
		...
		if (filename != null && filename.lastIndexOf('.') != -1) {
			...
			returnedType = android.webkit.MimeTypeMap.getSingleton()
					.getMimeTypeFromExtension(extension);
		}
		// If the MIME type set by the user's mailer is
		// application/octet-stream, try to figure
		// out whether there's a sane file type extension.
		if (returnedType != null
				&& !DEFAULT_ATTACHMENT_MIME_TYPE.equalsIgnoreCase(returnedType)) {
			return returnedType;
	 ...
	 * 
	 * @return If {@code mimeType} is known to be wrong the correct MIME type is
	 *         returned. Otherwise the lower case version of {@code mimeType} is
	 *         returned.
	 * 
	...
	public static String fixupCharset(String charset, Message message)
			throws MessagingException {
		if (charset == null || "0".equals(charset))
			charset = "US-ASCII"; // No encoding, so use us-ascii, which is the
									// standard.
	...
	private static String getJisVariantFromMessage(Message message)
			throws MessagingException {
		if (message == null)
			return null;

		// If a receiver is known to use a JIS variant, the sender transfers the
		// message after converting the
		// charset as a convention.
		String variant = getJisVariantFromReceivedHeaders(message);
		if (variant != null)
			return variant;

		// If a receiver is not known to use any JIS variants, the sender
		// transfers the message without converting
		// the charset.
	...
	private static String getJisVariantFromReceivedHeaders(Message message)
			throws MessagingException {
	...
	private static String getAddressFromReceivedHeader(String receivedHeader) {
		// Not implemented yet! Extract an address from the FOR clause of the
		// given Received header.
		return null;
	}
	...
	private static String getJisVariantFromAddress(String address) {
		if (address == null)
			return null;
		if (isInDomain(address, "docomo.ne.jp")
				|| isInDomain(address, "dwmail.jp")
				|| isInDomain(address, "pdx.ne.jp")
				|| isInDomain(address, "willcom.com")
				|| isInDomain(address, "emnet.ne.jp")
				|| isInDomain(address, "emobile.ne.jp"))
			return "docomo";
		else if (isInDomain(address, "softbank.ne.jp")
				|| isInDomain(address, "vodafone.ne.jp")
				|| isInDomain(address, "disney.ne.jp")
				|| isInDomain(address, "vertuclub.ne.jp"))
			return "softbank";
		else if (isInDomain(address, "ezweb.ne.jp")
				|| isInDomain(address, "ido.ne.jp"))
			return "kddi";
		return null;
	}
	...
	private static String getJisVariantFromMailerHeaders(Message message)
		...
		if (mailerHeaders[0].startsWith("iPhone Mail ")
				|| mailerHeaders[0].startsWith("iPad Mail "))
			return "iphone";
	...
	public static String readToString(InputStream in, String charset)
		...
		if (charset.length() > 19 && charset.startsWith("x-")
				&& charset.endsWith("-iso-2022-jp-2007")
				&& !Charset.isSupported(charset)) {
			in = new Iso2022JpToShiftJisInputStream(in);
			charset = "x-" + charset.substring(2, charset.length() - 17)
					+ "-shift_jis-2007";
		}
		...
		if (charset.length() > 17 && charset.startsWith("x-")
				...
				&& !Charset.isSupported(charset)) {
			// If the JIS variant is iPhone, map the Unicode private use area in
			// iPhone to the one in Android after
			// converting the character set from the standard Shift JIS to
			// Unicode.
		 ...
		 * this function may also throw an exception if the charset name is not
		 * known
		 */
		boolean supported;
		try {
	...
	public static void setCharset(String charset, Part part)
			throws MessagingException {
		part.setHeader(MimeHeader.HEADER_CONTENT_TYPE, part.getMimeType()
	...
	public static ViewableContainer extractPartsFromDraft(Message message)
		...
		if (message.isMimeType("multipart/mixed")
				&& body instanceof MimeMultipart) {
			MimeMultipart multipart = (MimeMultipart) body;

			ViewableContainer container;
			int count = multipart.getCount();
			if (count >= 1) {
				// The first part is either a text/plain or a
	...
	private static ViewableContainer extractTextual(Part part)
			throws MessagingException {
		String text = "";
		String html = "";
		List<Part> attachments = new ArrayList<Part>();

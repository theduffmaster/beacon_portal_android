 ...
 * 
 */
public class BinaryTempFileMessageBody extends BinaryTempFileBody implements
		CompositeBody {
	...
	public void writeTo(OutputStream out) throws IOException,
		...
		try {
			if (MimeUtil.ENC_7BIT.equalsIgnoreCase(mEncoding)) {
				/*
				 * If we knew the message was already 7bit clean, then it could
				 * be sent along without processing. But since we don't know, we
	...
	public void setUsing7bitTransport() throws MessagingException {
		/*
		 * There's nothing to recurse into here, so there's nothing to do. The
		 * enclosing BodyPart already called setEncoding(MimeUtil.ENC_7BIT).
		 * Once writeTo() is called, the file with the rfc822 body will be
		 * opened for reading and will then be recursed.
		 */

...
public class MimeMessage extends Message {
	 ...
	 * @param in
	 * @param recurse
	 *            A boolean indicating to recurse through all nested MimeMessage
	 *            subparts.
	 * @throws IOException
	 * @throws MessagingException
	 */
	public MimeMessage(InputStream in, boolean recurse) throws IOException,
			MessagingException {
		parse(in, recurse);
	}
	...
	protected void parse(InputStream in, boolean recurse) throws IOException,
			MessagingException {
		mHeader.clear();
		mFrom = null;
		mTo = null;
		...
		parserConfig.setMaxLineLen(-1); // The default is 1000 characters. Some
										// MUAs generate
		// REALLY long References: headers
		parserConfig.setMaxHeaderCount(-1); // Disable the check for header
											// count.
	...
	public Date getSentDate() {
		if (mSentDate == null) {
			try {
				DateTimeField field = (DateTimeField) DefaultFieldParser
						.parse("Date: "
								+ MimeUtility
										.unfoldAndDecode(getFirstHeader("Date")));
	 ...
	 * @see #mSentDate
	 * @param sentDate
	 * @throws com.bernard.beaconportal.activities.mail.MessagingException
	 */
	public void addSentDate(Date sentDate) throws MessagingException {
		if (mDateFormat == null) {
			mDateFormat = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z",
					Locale.US);
		}
	...
	public Address[] getReplyTo() {
		if (mReplyTo == null) {
			mReplyTo = Address.parse(MimeUtility
					.unfold(getFirstHeader("Reply-to")));
		}
		return mReplyTo;
	}
	...
	private String generateMessageId() {
		...
		/*
		 * We use upper case here to match Apple Mail Message-ID format (for
		 * privacy)
		 */
	...
	public void setReferences(String references) throws MessagingException {
		...
		// Make sure separator is SPACE to prevent Q-encoding when TAB is
		// encountered
		references = references.replaceAll("\\s+", " ");

		/*
		 * NOTE: Usually the maximum header line is 998 + CRLF = 1000
		 * characters. But at least one implementations seems to have problems
		 * with 998 characters, so we adjust for that fact.
		 */
		final int limit = 1000 - 2 /* CRLF */- 12 /* "References: " */- 1 /*
																			 * Off-
																			 * by
																			 * -
																			 * one
	...
	class MimeMessageBuilder implements ContentHandler {
		...
		public void field(Field parsedField) throws MimeException {
			expect(Part.class);
			try {
				((Part) stack.peek()).addHeader(parsedField.getName(),
						parsedField.getBody().trim());
			} catch (MessagingException me) {
				throw new Error(me);
			}
		}
	 ...
	 * 
	 * @param message
	 *            The {@code MimeMessage} object to receive the contents of this
	 *            instance.
	 */
	protected void copy(MimeMessage message) {
		...
		// These arrays are not supposed to be modified, so it's okay to reuse
		// the references
		message.mFrom = mFrom;
		message.mTo = mTo;
		message.mCc = mCc;

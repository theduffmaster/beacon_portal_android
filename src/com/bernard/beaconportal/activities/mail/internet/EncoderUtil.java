 ...
 * as defined in <a href='http://www.faqs.org/rfcs/rfc2047.html'>RFC 2047</a> or
 * display-names of an e-mail address, for example.
 * 
 * This class is copied from the org.apache.james.mime4j.decoder.EncoderUtil
 * class. It's modified here in order to encode emoji characters in the Subject
 * headers. The method to decode emoji depends on the MimeMessage class because
...
public class EncoderUtil {
	...
	private static String encodeB(String prefix, String text, Charset charset,
			byte[] bytes) {
		...
		if (totalLength <= ENCODED_WORD_MAX_LENGTH) {
			return prefix
					+ org.apache.james.mime4j.codec.EncoderUtil.encodeB(bytes)
					+ ENC_WORD_SUFFIX;
		} else {
	...
	private static String encodeQ(String prefix, String text, Charset charset,
			byte[] bytes) {
		...
		if (totalLength <= ENCODED_WORD_MAX_LENGTH) {
			return prefix
					+ org.apache.james.mime4j.codec.EncoderUtil
							.encodeQ(
									bytes,
									org.apache.james.mime4j.codec.EncoderUtil.Usage.WORD_ENTITY)
					+ ENC_WORD_SUFFIX;
		} else {
			String part1 = text.substring(0, text.length() / 2);
			byte[] bytes1 = encode(part1, charset);

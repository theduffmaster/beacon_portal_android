...
public abstract class Store {
	...
	/**
	 * Local stores indexed by UUID because the Uri may change due to migration
	 * to/from SD-card.
	 */
	 ...
	 */
	public synchronized static Store getRemoteInstance(Account account)
		...
		if (store == null) {
			throw new MessagingException(
					"Unable to locate an applicable Store for " + uri);
		}
	 ...
	 */
	public static LocalStore getLocalInstance(Account account,
			Application application) throws MessagingException {
		...
		// Use per-account locks so DatabaseUpgradeService always knows which
		// account database is
		// currently upgraded.
		synchronized (lock) {
			...
			if (store == null) {
				// Creating a LocalStore instance will create or upgrade the
				// database if
	 ...
	 * 
	 * @param account
	 *            {@link Account} instance that is used to get the local mail
	 *            store instance.
	 */
	 ...
	 * 
	 * @param account
	 *            {@link Account} instance that is used to get the remote mail
	 *            store instance.
	 */
	private synchronized static void removeRemoteInstance(Account account) {
		...
		if (uri.startsWith("local")) {
			throw new RuntimeException(
					"Asked to get non-local Store object but given "
							+ "LocalStore URI");
		}
	 ...
	 * {@link ServerSettings} object.
	 * 
	 * @param uri
	 *            the store-specific URI to decode
	 * 
	 * @return A {@link ServerSettings} object holding the settings contained in
	 *         the URI.
	 * 
	 ...
	 *            settings.
	 * 
	 * @return A store URI that holds the same information as the {@code server}
	 *         parameter.
	 * 
	...
	public abstract List<? extends Folder> getPersonalNamespaces(
			boolean forceListAll) throws MessagingException;

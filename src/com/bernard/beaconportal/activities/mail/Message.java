...
import com.bernard.beaconportal.activities.mail.filter.CountingOutputStream;
...
public abstract class Message implements Part, CompositeBody {
	...
	public boolean equals(Object o) {
		...
		return (mUid.equals(other.getUid())
				&& mFolder.getName().equals(other.getFolder().getName()) && mFolder
				.getAccount().getUuid()
	...
	public int hashCode() {
		...
		result = MULTIPLIER * result
				+ mFolder.getAccount().getUuid().hashCode();
	 ...
	 * Returns a message summary as a string suitable for showing in a message
	 * list
	 * 
	 * A message summary should be about the first 160 characters of unique text
	 * written by the message sender Quoted text, "On $date" and so on will be
	 * stripped out. All newlines and whitespace will be compressed.
	 */
	...
	public long calculateSize() {
		try {
			...
			Log.e(MAIL.LOG_TAG, "Failed to calculate a message size", e);
	 ...
	 * 
	 * @param destination
	 *            The {@code Message} object to receive the contents of this
	 *            instance.
	 */
	protected void copy(Message destination) {
		...
		// mFlags contents can change during the object lifetime, so copy the
		// Set
		destination.mFlags = new HashSet<Flag>(mFlags);
	}

	/**
	 * Creates a new {@code Message} object with the same content as this
	 * object.
	 * 
	 * <p>
	 * <strong>Note:</strong> This method was introduced as a hack to prevent
	 * {@code ConcurrentModificationException}s. It shouldn't be used unless
	 * absolutely necessary. See the comment in
	 * {@link com.bernard.beaconportal.activities.activity.MessageView.Listener#loadMessageForViewHeadersAvailable(com.bernard.beaconportal.activities.Account, String, String, Message)}

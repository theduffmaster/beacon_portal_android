...
public class FetchProfile extends ArrayList<FetchProfile.Item> {
	...
	public enum Item {
		 ...
		 * the size and the following headers: date, subject, from,
		 * content-type, to, cc
		 */
		ENVELOPE,
		 ...
		 * possible, fill in a properly formatted MIME structure in the message
		 * without actually downloading any message data. If the provider is not
		 * capable of this operation it should specifically set the body of the
		 * message to null so that upper levels can detect that a full body
		 * download is needed.
		 */
		STRUCTURE,

		/**
		 * A sane portion of the entire message, cut off at a provider
		 * determined limit. This should generaly be around 50kB.
		 */
		BODY_SANE,

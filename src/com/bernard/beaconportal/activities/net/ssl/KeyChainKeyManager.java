...
public class KeyChainKeyManager extends X509ExtendedKeyManager {
	...
	private static synchronized void savePrivateKeyReference(
	 ...
	 *             Indicates an error in retrieving the certificate for the
	 *             alias (likely because the alias is invalid or the certificate
	 *             was deleted)
	 */
	...
	private X509Certificate[] fetchCertificateChain(Context context,
			String alias) throws KeyChainException, InterruptedException,
			MessagingException {

		X509Certificate[] chain = KeyChain.getCertificateChain(context, alias);
		if (chain == null || chain.length == 0) {
			throw new MessagingException("No certificate chain found for: "
					+ alias);
		}
		try {
			...
			throw new CertificateValidationException(context.getString(
					R.string.client_certificate_expired, alias, e.toString()));
		}

		return chain;
	}

	private PrivateKey fetchPrivateKey(Context context, String alias)
		...
		/*
		 * We need to keep reference to the first private key retrieved so it
		 * won't get garbage collected. If it will then the whole app will crash
		 * on Android < 4.2 with "Fatal signal 11 code=1". See
	...
	private String chooseAlias(String[] keyTypes, Principal[] issuers) {
		...
		for (String keyAlgorithm : keyTypes) {
			...
			/*
			 * TODO find a more reliable test for signature algorithm.
			 * Unfortunately value varies with provider. For example for "EC" it
			 * could be "SHA1WithECDSA" or simply "ECDSA".
			 */
			...
			// check that a certificate in the chain was issued by one of the
			// specified issuers
			for (X509Certificate certFromChain : mChain) {
				/*
				 * Note use of X500Principal from getIssuerX500Principal as
				 * opposed to Principal from getIssuerDN. Principal.equals test
				 * does not work in the case where xcertFromChain.getIssuerDN is
				 * a bouncycastle org.bouncycastle.jce.X509Principal.
				 */
				X500Principal issuerFromChain = certFromChain
						.getIssuerX500Principal();
				if (issuersList.contains(issuerFromChain)) {
					return mAlias;
				}
			}
			Log.w(K9.LOG_TAG, "Client certificate " + mAlias
					+ " not issued by any of the requested issuers");
			return null;
		}
		Log.w(K9.LOG_TAG, "Client certificate " + mAlias
				+ " does not match any of the requested key types");
		return null;
	}
}

package com.bernard.beaconportal.activities.cache;
...
/**
 * Cache to bridge the time needed to write (user-initiated) changes to the
 * database.
 */
public class EmailProviderCache {
	...
	private static Map<String, EmailProviderCache> sInstances = new HashMap<String, EmailProviderCache>();
	...
	public void setValueForMessages(List<Long> messageIds, String columnName,
			String value) {
		synchronized (mMessageCache) {
	...
	public void setValueForThreads(List<Long> threadRootIds, String columnName,
			String value) {
		synchronized (mThreadCache) {
	...
	public void removeValueForThreads(List<Long> threadRootIds,
			String columnName) {
		synchronized (mThreadCache) {
			for (Long threadRootId : threadRootIds) {
				Map<String, String> map = mThreadCache.get(threadRootId);
	...
	public void hideMessages(List<Message> messages) {
		synchronized (mHiddenMessageCache) {
			for (Message message : messages) {
				...
				long folderId = ((LocalFolder) localMessage.getFolder())
						.getId();
	...
	public void unhideMessages(Message[] messages) {
		synchronized (mHiddenMessageCache) {
			for (Message message : messages) {
				...
				if (hiddenInFolder != null
						&& hiddenInFolder.longValue() == folderId) {
	 ...
	 * 
	 * <p>
	 * <strong>Note:</strong> Notifying the content resolver of the change will
	 * cause the {@code CursorLoader} in {@link MessageListFragment} to reload
	 * the cursor. But especially with flag changes this will block because of
	 * the DB write operation to update the flags. So additionally we use
	 * {@link LocalBroadcastManager} to send a {@link #ACTION_CACHE_UPDATED}
	 * broadcast. This way {@code MessageListFragment} can update the view
	 * without reloading the cursor.
	 * </p>
	 */
	private void notifyChange() {
		LocalBroadcastManager.getInstance(sContext).sendBroadcast(
				new Intent(ACTION_CACHE_UPDATED));

		Uri uri = Uri.withAppendedPath(EmailProvider.CONTENT_URI, "account/"
				+ mAccountUuid + "/messages");

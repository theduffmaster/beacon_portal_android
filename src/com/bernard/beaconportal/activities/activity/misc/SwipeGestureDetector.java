...
public class SwipeGestureDetector extends SimpleOnGestureListener {
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		// Calculate the minimum distance required for this to be considered a
		// swipe.
		final int minDistance = (int) Math.abs(deltaY * 4);

		try {
			if (Math.abs(deltaY) > mMaxOffPath
			...
			// successful fling, cancel the 2nd event to prevent any other
	...
	/**
	 * A listener that will be notified when a right to left or left to right
	 * swipe has been detected.
	 */
	public interface OnSwipeGestureListener {
		/**
		 * Called when a swipe from right to left is handled by
		 * {@link MyGestureDetector}.
		 * 
		 * <p>
		 * See
		 * {@link OnGestureListener#onFling(MotionEvent, MotionEvent, float, float)}
		...
		/**
		 * Called when a swipe from left to right is handled by
		 * {@link MyGestureDetector}.
		 * 
		 * <p>
		 * See

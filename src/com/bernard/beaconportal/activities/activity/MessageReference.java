...
public class MessageReference implements Parcelable {
	...
	// Version identifier for use when serializing. This will allow us to
	// introduce future versions
	 ...
	 *             On missing or corrupted identity.
	 */
	public MessageReference(final String identity) throws MessagingException {
		// Can't be null and must be at least length one so we can check the
		// version.
		if (identity == null || identity.length() < 1) {
			throw new MessagingException(
					"Null or truncated MessageReference identity.");
		}

		// Version check.
		if (identity.charAt(0) == IDENTITY_VERSION_1.charAt(0)) {
			// Split the identity, stripping away the first two characters
			// representing the version and delimiter.
			StringTokenizer tokens = new StringTokenizer(identity.substring(2),
					IDENTITY_SEPARATOR, false);
			if (tokens.countTokens() >= 3) {
				...
				if (tokens.hasMoreTokens()) {
					...
					try {
						...
						throw new MessagingException(
								"Could not thaw message flag '" + flagString
										+ "'", ie);
					}
				}
	...
	public boolean equals(Object o) {
		...
		if ((accountUuid == other.accountUuid || (accountUuid != null && accountUuid
				.equals(other.accountUuid)))
				&& (folderName == other.folderName || (folderName != null && folderName
						.equals(other.folderName)))
	...
	public String toString() {
		return "MessageReference{" + "accountUuid='" + accountUuid + '\''

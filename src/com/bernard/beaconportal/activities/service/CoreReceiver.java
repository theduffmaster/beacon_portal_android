...
public class CoreReceiver extends BroadcastReceiver {
	...
	private static void releaseWakeLock(Integer wakeLockId) {
		if (wakeLockId != null) {
			...
			if (wl != null) {
				if (K9.DEBUG)
					Log.v(K9.LOG_TAG, "CoreReceiver Releasing wakeLock "
							+ wakeLockId);
				wl.release();
			} else {
				Log.w(K9.LOG_TAG, "BootReceiver WakeLock " + wakeLockId
						+ " doesn't exist");
			}
		}
	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			...
			if (CoreReceiver.WAKE_LOCK_RELEASE.equals(intent.getAction())) {
				...
				if (wakeLockId != -1) {
					if (K9.DEBUG)
						Log.v(K9.LOG_TAG, "CoreReceiver Release wakeLock "
								+ wakeLockId);
	...
	public static void releaseWakeLock(Context context, int wakeLockId) {
		if (K9.DEBUG)
			Log.v(K9.LOG_TAG, "CoreReceiver Got request to release wakeLock "
					+ wakeLockId);
		Intent i = new Intent();

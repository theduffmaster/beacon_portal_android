...
public class NotificationActionService extends CoreService {
	...
	private final static String READ_ALL_ACTION = "com.bernard.beaconportal.activities.service.NotificationActionService.READ_ALL_ACTION";
	private final static String DELETE_ALL_ACTION = "com.bernard.beaconportal.activities.service.NotificationActionService.DELETE_ALL_ACTION";
	private final static String ACKNOWLEDGE_ACTION = "com.bernard.beaconportal.activities.service.NotificationActionService.ACKNOWLEDGE_ACTION";

	private final static String EXTRA_ACCOUNT = "account";
	private final static String EXTRA_MESSAGE = "message";
	private final static String EXTRA_MESSAGE_LIST = "messages";

	public static PendingIntent getReplyIntent(Context context,
			final Account account, final MessageReference ref) {
		Intent i = new Intent(context, NotificationActionService.class);
		i.putExtra(EXTRA_ACCOUNT, account.getUuid());
		i.putExtra(EXTRA_MESSAGE, ref);
		i.setAction(REPLY_ACTION);

		return PendingIntent.getService(context, account.getAccountNumber(), i,
				PendingIntent.FLAG_UPDATE_CURRENT);
	}

	public static PendingIntent getReadAllMessagesIntent(Context context,
			final Account account, final ArrayList<MessageReference> refs) {
		Intent i = new Intent(context, NotificationActionService.class);
		i.putExtra(EXTRA_ACCOUNT, account.getUuid());
	...
	public static PendingIntent getAcknowledgeIntent(Context context,
	...
	public static Intent getDeleteAllMessagesIntent(Context context,
			final Account account, final ArrayList<MessageReference> refs) {
		Intent i = new Intent(context, NotificationActionService.class);
		i.putExtra(EXTRA_ACCOUNT, account.getUuid());
		i.putExtra(EXTRA_MESSAGE_LIST, refs);
	...
	public int startService(Intent intent, int startId) {
		...
		final Account account = preferences.getAccount(intent
		...
		if (account != null) {
			if (READ_ALL_ACTION.equals(action)) {
				...
				ArrayList<MessageReference> refs = intent
						.getParcelableArrayListExtra(EXTRA_MESSAGE_LIST);
				for (MessageReference ref : refs) {
					controller.setFlag(account, ref.folderName, ref.uid,
							Flag.SEEN, true);
				}
				...
				MessageReference ref = (MessageReference) intent
						.getParcelableExtra(EXTRA_MESSAGE);
				Message message = ref.restoreToLocalMessage(this);
				if (message != null) {
					Intent i = MessageCompose.getActionReplyIntent(this,
							account, message, false, null);
			...
			/*
			 * there's no point in keeping the notification after the user
			 * clicked on it
			 */

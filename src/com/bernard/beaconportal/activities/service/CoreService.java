 ...
 * </p>
 * <p>
 * A service's main purpose is to do some task (usually in the background) which
 * requires one or more threads. The thread that starts the service is the same
 * as the UI thread of the process. It should thus not be used to run the tasks.
 * </p>
 * <p>
 ...
 * that work is active processing or some just some monitoring, like listening
 * on a network port for incoming connections or listing on an open network
 * connection for incoming data (push mechanism).
 * </p>
 * <p>
 * To make sure the service is running only when required, is must be shutdown
 * after tasks are done. As the execution of tasks is abstracted away in this
 * class, it also handles proper shutdown. If a Service doesn't want this, it
 * needs to call {@code enableAutoShutdown(true)} in its
 * {@link Service#onCreate()} method.
 * </p>
 * <p>
 * While a service is running its tasks, it is usually not a good idea to let
 * the device go to sleep mode. Wake locks are used to avoid this. CoreService
 * provides a central registry (singleton) that can be used application-wide to
 * store wake locks.
 * </p>
 * <p>
 ...
 * needed anymore); enabled by default</li>
 * <li>wake lock registry and management</li>
 * </ul>
 */
public abstract class CoreService extends Service {

	public static String WAKE_LOCK_ID = "com.bernard.beaconportal.activities.service.CoreService.wakeLockId";

	private static ConcurrentHashMap<Integer, TracingWakeLock> sWakeLocks = new ConcurrentHashMap<Integer, TracingWakeLock>();
	private static AtomicInteger sWakeLockSeq = new AtomicInteger(0);

	/**
	 * Threadpool that is used to execute and queue background actions inside
	 * the service.
	 */
	...
	/**
	 * {@code true} if the {@code Service}'s {@link #onDestroy()} method was
	 * called. {@code false} otherwise.
	 * 
	 * <p>
	 * <strong>Note:</strong> This is used to ignore (expected)
	 * {@link RejectedExecutionException}s thrown by
	 * {@link ExecutorService#execute(Runnable)} after the service (and with
	 * that, the thread pool) was shut down.
	 * </p>
	 */
	 ...
	 * 
	 * <p>
	 * The default service life-cycle model is that the service should run only
	 * as long as a task is running. If a service should behave differently,
	 * disable auto shutdown using {@link #setAutoShutdown(boolean)}.
	 * </p>
	 */
	private boolean mAutoShutdown = true;

	/**
	 * This variable is part of the auto shutdown feature and determines whether
	 * the service has to be shutdown at the end of the
	 * {@link #onStart(Intent, int)} method or not.
	 */
	protected boolean mImmediateShutdown = true;

	/**
	 * Adds an existing wake lock identified by its registry ID to the specified
	 * intent.
	 * 
	 * @param context
	 *            A {@link Context} instance. Never {@code null}.
	 * @param intent
	 *            The {@link Intent} to add the wake lock registy ID as extra
	 *            to. Never {@code null}.
	 * @param wakeLockId
	 *            The wake lock registry ID of an existing wake lock or
	 *            {@code null}.
	 * @param createIfNotExists
	 *            If {@code wakeLockId} is {@code null} and this parameter is
	 *            {@code true} a new wake lock is created, registered, and added
	 *            to {@code intent}.
	 */
	protected static void addWakeLockId(Context context, Intent intent,
	 ...
	 * 
	 * <p>
	 * This will add the wake lock to the central wake lock registry managed by
	 * this class.
	 * </p>
	 * 
	 * @param context
	 ...
	 * 
	 * @param wakeLock
	 *            The {@link TracingWakeLock} instance that should be registered
	 *            with the wake lock registry. Never {@code null}.
	 * 
	 ...
	 */
	protected static TracingWakeLock acquireWakeLock(Context context,
			String tag, long timeout) {
	...
	public final int onStartCommand(Intent intent, int flags, int startId) {
		/*
		 * When a process is killed due to low memory, it's later restarted and
		 * services that were started with START_STICKY are started with the
		 * intent being null.
		 * 
		...
		try {
			...
			try {
				// If there is no outstanding work to be done in a background
				// thread we can stop
				// this service.
	 ...
	 * @param wakeLockTime
	 *            The timeout for the wake lock that will be acquired by this
	 *            method.
	 * @param startId
	 *            The {@code startId} value received in
	 *            {@link #onStart(Intent, int)} or {@code null} if you don't
	 *            want the service to be shut down after {@code runner} has been
	 *            executed (e.g. because you need to run another background
	 *            task).<br>
	 *            If this parameter is {@code null} you need to call
	 *            {@code setAutoShutdown(false)} otherwise the auto shutdown
	 *            code will stop the service.
	 */
	public void execute(Context context, final Runnable runner,
			int wakeLockTime, final Integer startId) {

		boolean serviceShutdownScheduled = false;
		final boolean autoShutdown = mAutoShutdown;

		// Acquire a new wakelock
		final TracingWakeLock wakeLock = acquireWakeLock(context,
				"CoreService execute", wakeLockTime);

		// Wrap the supplied runner with code to release the wake lock and stop
		// the service if
		// appropriate.
		Runnable myRunner = new Runnable() {
			...
			public void run() {
				try {
					...
					// If the sync status changed while runner was executing,
					// notify
					// MessagingController
		...
		if (mThreadPool == null) {
			...
			try {
				...
				// Ignore RejectedExecutionException after we shut down the
				// thread pool in
	...
	/**
	 * Subclasses need to implement this instead of overriding
	 * {@link #onStartCommand(Intent, int, int)}.
	 * 
	 * <p>
	 * This allows {@link CoreService} to manage the service lifecycle, incl.
	 ...
	 * 
	 * @param autoShutdown
	 *            {@code true} to enable auto shutdown. {@code false} to
	 *            disable.
	 * 
	 * @see #mAutoShutdown
	 */

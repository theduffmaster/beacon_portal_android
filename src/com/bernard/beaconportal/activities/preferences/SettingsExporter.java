...
public class SettingsExporter {
	 ...
	 * 
	 * <p>
	 * Increment this if you need to change the structure of the settings file.
	 * When you do this remember that we also have to be able to handle old file
	 * formats. So have fun adding support for that to {@link SettingsImporter}
	 * :)
	 * </p>
	 */
	...
	public static void exportPreferences(Context context, OutputStream os,
		...
		try {
			...
			serializer.attribute(null, VERSION_ATTRIBUTE,
					Integer.toString(Settings.VERSION));
			...
			Map<String, Object> prefs = new TreeMap<String, Object>(
					storage.getAll());
	...
	private static void writeSettings(XmlSerializer serializer,
		...
		for (Entry<String, TreeMap<Integer, SettingsDescription>> versionedSetting : GlobalSettings.SETTINGS
				.entrySet()) {
	...
	private static void writeAccount(XmlSerializer serializer, Account account,
		...
		String name = (String) prefs.get(accountUuid + "."
				+ Account.ACCOUNT_DESCRIPTION_KEY);
		if (name != null) {
		...
		ServerSettings outgoing = Transport.decodeTransportUri(account
				.getTransportUri());
		...
		for (Map.Entry<String, Object> entry : prefs.entrySet()) {
			...
			if (!keyUuid.equals(accountUuid)) {
				// Setting doesn't belong to the account we're currently
				// writing.
				continue;
			}
			...
			TreeMap<Integer, SettingsDescription> versionedSetting = AccountSettings.SETTINGS
					.get(keyPart);

			if (versionedSetting != null) {
				Integer highestVersion = versionedSetting.lastKey();
				SettingsDescription setting = versionedSetting
						.get(highestVersion);

				if (setting != null) {
					// Only export account settings that can be found in
		...
		if (identities.size() > 0) {
			...
			for (Integer identityIndex : sortedIdentities) {
				writeIdentity(serializer, accountUuid,
						identityIndex.toString(), prefs);
			}
	...
	private static void writeIdentity(XmlSerializer serializer,
			String accountUuid, String identity, Map<String, Object> prefs)
			throws IOException {
		...
		String name = (String) prefs.get(prefix + Account.IDENTITY_NAME_KEY
				+ suffix);
		serializer.startTag(null, NAME_ELEMENT);
		serializer.text(name);
		serializer.endTag(null, NAME_ELEMENT);
		...
		String email = (String) prefs.get(prefix + Account.IDENTITY_EMAIL_KEY
				+ suffix);
		...
		String description = (String) prefs.get(prefix
				+ Account.IDENTITY_DESCRIPTION_KEY + suffix);
		if (description != null) {
		...
		for (Map.Entry<String, Object> entry : prefs.entrySet()) {
			...
			TreeMap<Integer, SettingsDescription> versionedSetting = IdentitySettings.SETTINGS
					.get(identityKey);
			...
			if (versionedSetting != null) {
				...
				if (setting != null) {
					// Only write settings that have an entry in
	...
	private static void writeFolder(XmlSerializer serializer,
			String accountUuid, String folder, Map<String, Object> prefs)
			throws IOException {
		...
		for (Map.Entry<String, Object> entry : prefs.entrySet()) {
			...
			TreeMap<Integer, SettingsDescription> versionedSetting = FolderSettings.SETTINGS
					.get(folderKey);
	...
	private static void writeElement(XmlSerializer serializer,
			String elementName, String value) throws IllegalArgumentException,
	...
	private static void writeKeyValue(XmlSerializer serializer, String key,

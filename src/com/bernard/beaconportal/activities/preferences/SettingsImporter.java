...
public class SettingsImporter {
	...
	public static class ImportContents {
		...
		private ImportContents(boolean globalSettings,
	...
	public static class AccountDescriptionPair {
		...
		private AccountDescriptionPair(AccountDescription original,
				AccountDescription imported, boolean overwritten) {
	...
	/**
	 * Parses an import {@link InputStream} and returns information on whether
	 * it contains global settings and/or account settings. For all account
	 * configurations found, the name of the account along with the account UUID
	 * is returned.
	 * 
	 * @param inputStream
	 *            An {@code InputStream} to read the settings from.
	 * 
	 * @return An {@link ImportContents} instance containing information about
	...
	public static ImportContents getImportStreamContents(InputStream inputStream)
		...
		try {
			// Parse the import stream but don't save individual settings
			// (overview=true)
			Imported imported = parseSettings(inputStream, false, null, true);

			// If the stream contains global settings the "globalSettings"
			// member will not be null
			...
			// If the stream contains at least one account configuration the
			// "accounts" member
			// will not be null.
			if (imported.accounts != null) {
				for (ImportedAccount account : imported.accounts.values()) {
					accounts.add(new AccountDescription(account.name,
							account.uuid));
				}
			}

			// TODO: throw exception if neither global settings nor account
	...
	/**
	 * Reads an import {@link InputStream} and imports the global settings
	 * and/or account configurations specified by the arguments.
	 * 
	 * @param context
	 ...
	 *            {@code true} if global settings should be imported from the
	 *            file.
	 * @param accountUuids
	 *            A list of UUIDs of the accounts that should be imported.
	 * @param overwrite
	 *            {@code true} if existing accounts should be overwritten when
	 *            an account with the same UUID is found in the settings file.<br>
	 *            <strong>Note:</strong> This can have side-effects we currently
	 *            don't handle, e.g. changing the account type from IMAP to
	 *            POP3. So don't use this for now!
	 * 
	 * @return An {@link ImportResults} instance containing information about
	 *         errors and successfully imported accounts.
	 * 
	 * @throws SettingsImportExportException
	 *             In case of an error.
	 */
	public static ImportResults importSettings(Context context,
			InputStream inputStream, boolean globalSettings,
		...
		try {
			...
			Imported imported = parseSettings(inputStream, globalSettings,
					accountUuids, false);
			...
			if (globalSettings) {
				try {
					...
					if (imported.globalSettings != null) {
						importGlobalSettings(storage, editor,
				...
				if (imported.accounts != null) {
					for (String accountUuid : accountUuids) {
						if (imported.accounts.containsKey(accountUuid)) {
							ImportedAccount account = imported.accounts
									.get(accountUuid);
							try {
								SharedPreferences.Editor editor = storage
										.edit();

								AccountDescriptionPair importResult = importAccount(
										context, editor,
										imported.contentVersion, account,
										overwrite);
								...
								if (editor.commit()) {
									...
									// Add UUID of the account we just imported
									// to the list of
									// account UUIDs
									if (!importResult.overwritten) {
										...
										String oldAccountUuids = storage
												.getString("accountUuids", "");
										String newAccountUuids = (oldAccountUuids
												.length() > 0) ? oldAccountUuids
												+ "," + newUuid
												: newUuid;

										putString(editor, "accountUuids",
												newAccountUuids);

										if (!editor.commit()) {
											throw new SettingsImportExportException(
													"Failed to set account UUID list");
										}
									}
					...
					String defaultAccountUuid = storage.getString(
							"defaultAccountUuid", null);
					if (defaultAccountUuid == null) {
						putString(editor, "defaultAccountUuid",
								accountUuids.get(0));
					}
	...
	private static void importGlobalSettings(SharedPreferences storage,
			SharedPreferences.Editor editor, int contentVersion,
			ImportedSettings settings) {

		// Validate global settings
		Map<String, Object> validatedSettings = GlobalSettings.validate(
				contentVersion, settings.settings);
		...
		// Convert global settings to the string representation used in
		// preference storage
		Map<String, String> stringSettings = GlobalSettings
				.convert(validatedSettings);

		// Use current global settings as base and overwrite with validated
		// settings read from the
		// import file.
		Map<String, String> mergedSettings = new HashMap<String, String>(
				GlobalSettings.getGlobalSettings(storage));
	...
	private static AccountDescriptionPair importAccount(Context context,
		...
		AccountDescription original = new AccountDescription(account.name,
				account.uuid);
		...
		if (!overwrite && existingAccount != null) {
			// An account with this UUID already exists, but we're not allowed
			// to overwrite it.
		...
		if (isAccountNameUsed(accountName, accounts)) {
			// Account name is already in use. So generate a new one by
			// appending " (x)", where x
		...
		putString(editor, accountKeyPrefix + Account.ACCOUNT_DESCRIPTION_KEY,
				accountName);
		...
		if (account.outgoing == null
			...
			// All account types except WebDAV need to provide outgoing server
			// settings
			throw new InvalidSettingValueException();
		}
		...
		if (account.outgoing != null) {
			...
			ServerSettings outgoing = new ImportedServerSettings(
					account.outgoing);
		...
		Map<String, Object> validatedSettings = AccountSettings.validate(
				contentVersion, account.settings.settings,
		...
		// Convert account settings to the string representation used in
		// preference storage
		Map<String, String> stringSettings = AccountSettings
				.convert(validatedSettings);
		...
		if (mergeImportedAccount) {
			writeSettings = new HashMap<String, String>(
					AccountSettings.getAccountSettings(prefs.getPreferences(),
							uuid));
		...
		if (!mergeImportedAccount) {
			...
			putString(editor, accountKeyPrefix + "accountNumber",
					Integer.toString(newAccountNumber));
		}

		// Write identities
		if (account.identities != null) {
			importIdentities(editor, contentVersion, uuid, account, overwrite,
					existingAccount, prefs);
	...
	private static void importFolder(SharedPreferences.Editor editor,
			int contentVersion, String uuid, ImportedFolder folder,
			boolean overwrite, Preferences prefs) {

		// Validate folder settings
		Map<String, Object> validatedSettings = FolderSettings.validate(
				contentVersion, folder.settings.settings, !overwrite);
		...
		// Convert folder settings to the string representation used in
		// preference storage
		Map<String, String> stringSettings = FolderSettings
				.convert(validatedSettings);

		// Merge folder settings if necessary
		Map<String, String> writeSettings;
		if (overwrite) {
			writeSettings = FolderSettings.getFolderSettings(
					prefs.getPreferences(), uuid, folder.name);
			writeSettings.putAll(stringSettings);
		} else {
			writeSettings = stringSettings;
		}
	...
	private static void importIdentities(SharedPreferences.Editor editor,
			int contentVersion, String uuid, ImportedAccount account,
			boolean overwrite, Account existingAccount, Preferences prefs)
			throws InvalidSettingValueException {

		String accountKeyPrefix = uuid + ".";

		// Gather information about existing identities for this account (if
		// any)
		...
		for (ImportedIdentity identity : account.identities) {
			...
			String identityDescription = (identity.description == null) ? "Imported"
					: identity.description;
			if (isIdentityDescriptionUsed(identityDescription,
					existingIdentities)) {
				// Identity description is already in use. So generate a new one
				// by appending
				// " (x)", where x is the first number >= 1 that results in an
				// unused identity
				// description.
			...
			putString(editor, accountKeyPrefix + Account.IDENTITY_NAME_KEY
			...
			// Write email address
			putString(editor, accountKeyPrefix + Account.IDENTITY_EMAIL_KEY
					+ identitySuffix, identity.email);

			// Write identity description
			putString(editor, accountKeyPrefix
					+ Account.IDENTITY_DESCRIPTION_KEY + identitySuffix,
					identityDescription);

			if (identity.settings != null) {
				// Validate identity settings
				Map<String, Object> validatedSettings = IdentitySettings
						.validate(contentVersion, identity.settings.settings,
								!mergeSettings);
				...
				// Convert identity settings to the representation used in
				// preference storage
				Map<String, String> stringSettings = IdentitySettings
						.convert(validatedSettings);
				...
				for (Map.Entry<String, String> setting : writeSettings
						.entrySet()) {
					String key = accountKeyPrefix + setting.getKey()
							+ identitySuffix;
					String value = setting.getValue();
	...
	private static boolean isIdentityDescriptionUsed(String description,
	...
	/**
	 * Write to an {@link SharedPreferences.Editor} while logging what is
	 * written if debug logging is enabled.
	 * 
	 * @param editor
	...
	private static Imported parseSettings(InputStream inputStream,
			boolean globalSettings, List<String> accountUuids, boolean overview)
			throws SettingsImportExportException {

		if (!overview && accountUuids == null) {
			throw new IllegalArgumentException(
					"Argument 'accountUuids' must not be null.");
		}
		...
		try {
			...
			if (imported == null
					|| (overview && imported.globalSettings == null && imported.accounts == null)) {
	...
	private static void skipToEndTag(XmlPullParser xpp, String endTag)
		...
		while (!(eventType == XmlPullParser.END_TAG && endTag.equals(xpp
				.getName()))) {
			eventType = xpp.next();
		}
	}
	...
	private static Imported parseRoot(XmlPullParser xpp,
			boolean globalSettings, List<String> accountUuids, boolean overview)
			throws XmlPullParserException, IOException,
		...
		int eventType = xpp.next();
		while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.ROOT_ELEMENT
				.equals(xpp.getName()))) {
					...
					if (overview || globalSettings) {
						if (result.globalSettings == null) {
							if (overview) {
								...
								result.globalSettings = parseSettings(xpp,
										SettingsExporter.GLOBAL_ELEMENT);
							}
						} else {
	...
	private static int validateFileFormatVersion(String versionString)
		...
		try {
			...
			throw new SettingsImportExportException(
					"Invalid file format version: " + versionString);
		}
	...
	private static int validateContentVersion(String versionString)
		...
		if (version < 1 || version > Settings.VERSION) {
			throw new SettingsImportExportException(
					"Unsupported content version: " + versionString);
		}

		return version;
	}

	private static ImportedSettings parseSettings(XmlPullParser xpp,
		...
		while (!(eventType == XmlPullParser.END_TAG && endTag.equals(xpp
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (SettingsExporter.VALUE_ELEMENT.equals(element)) {
					String key = xpp.getAttributeValue(null,
							SettingsExporter.KEY_ATTRIBUTE);
	...
	private static Map<String, ImportedAccount> parseAccounts(
		...
		int eventType = xpp.next();
		while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.ACCOUNTS_ELEMENT
				.equals(xpp.getName()))) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (SettingsExporter.ACCOUNT_ELEMENT.equals(element)) {
					...
					ImportedAccount account = parseAccount(xpp, accountUuids,
							overview);
	...
	private static ImportedAccount parseAccount(XmlPullParser xpp,
			List<String> accountUuids, boolean overview)
			throws XmlPullParserException, IOException {

		String uuid = xpp.getAttributeValue(null,
				SettingsExporter.UUID_ATTRIBUTE);
		...
		if (overview || accountUuids.contains(uuid)) {
			int eventType = xpp.next();
			while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.ACCOUNT_ELEMENT
					.equals(xpp.getName()))) {
				...
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (SettingsExporter.NAME_ELEMENT.equals(element)) {
						...
						if (overview) {
							...
							account.incoming = parseServerSettings(xpp,
									SettingsExporter.INCOMING_SERVER_ELEMENT);
						}
					} else if (SettingsExporter.OUTGOING_SERVER_ELEMENT
							.equals(element)) {
						if (overview) {
							skipToEndTag(xpp,
									SettingsExporter.OUTGOING_SERVER_ELEMENT);
						} else {
							account.outgoing = parseServerSettings(xpp,
									SettingsExporter.OUTGOING_SERVER_ELEMENT);
						}
					} else if (SettingsExporter.SETTINGS_ELEMENT
							.equals(element)) {
						if (overview) {
							skipToEndTag(xpp, SettingsExporter.SETTINGS_ELEMENT);
						} else {
							account.settings = parseSettings(xpp,
									SettingsExporter.SETTINGS_ELEMENT);
						}
					} else if (SettingsExporter.IDENTITIES_ELEMENT
							.equals(element)) {
						if (overview) {
							skipToEndTag(xpp,
	...
	private static ImportedServer parseServerSettings(XmlPullParser xpp,
			String endTag) throws XmlPullParserException, IOException {
		ImportedServer server = new ImportedServer();

		server.type = xpp.getAttributeValue(null,
				SettingsExporter.TYPE_ATTRIBUTE);
		...
		while (!(eventType == XmlPullParser.END_TAG && endTag.equals(xpp
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				} else if (SettingsExporter.CONNECTION_SECURITY_ELEMENT
						.equals(element)) {
					server.connectionSecurity = getText(xpp);
				} else if (SettingsExporter.AUTHENTICATION_TYPE_ELEMENT
						.equals(element)) {
	...
	private static List<ImportedIdentity> parseIdentities(XmlPullParser xpp)
		...
		int eventType = xpp.next();
		while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.IDENTITIES_ELEMENT
				.equals(xpp.getName()))) {
	...
	private static ImportedIdentity parseIdentity(XmlPullParser xpp)
		...
		int eventType = xpp.next();
		while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.IDENTITY_ELEMENT
				.equals(xpp.getName()))) {
	...
	private static List<ImportedFolder> parseFolders(XmlPullParser xpp)
		...
		int eventType = xpp.next();
		while (!(eventType == XmlPullParser.END_TAG && SettingsExporter.FOLDERS_ELEMENT
				.equals(xpp.getName()))) {
	...
	private static ImportedFolder parseFolder(XmlPullParser xpp)
		...
		String name = xpp.getAttributeValue(null,
				SettingsExporter.NAME_ATTRIBUTE);
		folder.name = name;
	...
	private static class ImportedServerSettings extends ServerSettings {
		...
		public Map<String, String> getExtra() {
			return (mImportedServer.extras != null) ? Collections
					.unmodifiableMap(mImportedServer.extras.settings) : null;
		}
		...
		private static ConnectionSecurity convertConnectionSecurity(
				String connectionSecurity) {
			try {
				/*
				 * TODO: Add proper settings validation and upgrade capability
				 * for server settings. Once that exists, move this code into a
				 * SettingsUpgrader.
				 */

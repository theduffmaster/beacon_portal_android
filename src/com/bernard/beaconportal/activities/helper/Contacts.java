...
public class Contacts {
	 ...
	 */
	protected static final String SORT_ORDER = ContactsContract.CommonDataKinds.Email.TIMES_CONTACTED
			+ " DESC, "
	 ...
	 * {@link com.bernard.beaconportal.activities.EmailAddressAdapter} or more
	 * specificly by {@link android.widget.ResourceCursorAdapter}.
	 */
	...
	/**
	 * Index of the contact id field in the projection. This must match the
	 * order in {@link #PROJECTION}.
	 */
	 ...
	 * 
	 * <p>
	 * Note: This is left over from the days when we needed to have SDK-specific
	 * code to access the contacts.
	 * </p>
	 * 
	 * @param context
	...
	/**
	 * Start the activity to add information to an existing contact or add a new
	 * one.
	 * 
	 * @param email
	 *            An {@link Address} instance containing the email address of
	 *            the entity you want to add to the contacts. Optionally the
	 *            instance also contains the (display) name of that entity.
	 */
	public void createContact(final Address email) {
		final Uri contactUri = Uri
				.fromParts("mailto", email.getAddress(), null);
	...
	/**
	 * Start the activity to add a phone number to an existing contact or add a
	 * new one.
	 * 
	 * @param phoneNumber
	 *            The phone number to add to a contact, or to use when creating
	 *            a new contact.
	 */
	public void addPhoneContact(final String phoneNumber) {
		Intent addIntent = new Intent(Intent.ACTION_INSERT_OR_EDIT);
		addIntent.setType(ContactsContract.Contacts.CONTENT_ITEM_TYPE);
		addIntent.putExtra(ContactsContract.Intents.Insert.PHONE,
				Uri.decode(phoneNumber));
	 ...
	 * 
	 * @param constraint
	 *            The search term to filter the contacts.
	 * @return A {@link Cursor} instance that can be used to get the matching
	 *         contacts.
	 */
	public Cursor searchContacts(final CharSequence constraint) {
		...
		if (c != null) {
			/*
			 * To prevent expensive execution in the UI thread: Cursors get
			 * lazily executed, so if you don't call anything on the cursor
			 * before returning it from the background thread you'll have a
			 * complied program for the cursor, but it won't have been executed
			 * to generate the data yet. Often the execution is more expensive
	 ...
	 * 
	 * @param addresses
	 *            Array of {@link Address} objects describing the contacts to be
	 *            marked as contacted.
	 */
		...
		for (final Address address : addresses) {
			...
			if (c != null) {
				if (c.getCount() > 0) {
					...
					ContactsContract.Contacts.markAsContacted(mContentResolver,
							personId);
				}
				c.close();
			}
	...
	/**
	 * Given a contact picker intent, returns a {@code ContactItem} instance for
	 * that contact.
	 * 
	 * @param intent
	 *            The {@link Intent} returned by the contact picker.
	 * 
	 * @return A {@link ContactItem} instance describing the picked contact. Or
	 *         {@code null} if the contact doesn't have any email addresses.
	 */
	 ...
	 * 
	 * @param address
	 *            An email address. The contact database is searched for a
	 *            contact with this email address.
	 * 
	 * @return URI to the picture of the contact with the supplied email
	 *         address. {@code null} if no such contact could be found or the
	...
	/**
	 * Does the device actually have a Contacts application suitable for picking
	 * a contact. As hard as it is to believe, some vendors ship without it.
	 * 

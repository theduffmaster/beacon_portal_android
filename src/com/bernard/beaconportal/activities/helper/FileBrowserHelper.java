...
public class FileBrowserHelper {
	/**
	 * A string array that specifies the name of the intent to use, and the
	 * scheme to use with it when setting the data for the intent.
	 */
	private static final String[][] PICK_DIRECTORY_INTENTS = {
			{ "org.openintents.action.PICK_DIRECTORY", "file://" }, // OI File
																	// Manager
																	// (maybe
																	// others)
			{ "com.estrongs.action.PICK_DIRECTORY", "file://" }, // ES File
																	// Explorer
			{ Intent.ACTION_PICK, "folder://" }, // Blackmoon File Browser
													// (maybe others)
	...
	/**
	 * tries to open known filebrowsers. If no filebrowser is found and fallback
	 * textdialog is shown
	 * 
	 * @param c
	 *            the context as activity
	 * @param startPath
	 *            : the default value, where the filebrowser will start. if
	 *            startPath = null => the default path is used
	 * @param requestcode
	 *            : the int you will get as requestcode in onActivityResult
	 *            (only used if there is a filebrowser installed)
	 * @param callback
	 *            : the callback (only used when no filebrowser is installed. if
	 *            a filebrowser is installed => override the onActivtyResult
	 *            Method
	 * 
	 * @return true: if a filebrowser has been found (the result will be in the
	 *         onActivityResult false: a fallback textinput has been shown. The
	 *         Result will be sent with the callback method
	 * 
	 * 
	 */
	public boolean showFileBrowserActivity(Activity c, File startPath,
			int requestcode, FileBrowserFailOverCallback callback) {
	...
	public boolean showFileBrowserActivity(Fragment c, File startPath,
			int requestcode, FileBrowserFailOverCallback callback) {
		boolean success = false;
	...
	private void showPathTextInput(final Activity c, final File startPath,
			final FileBrowserFailOverCallback callback) {

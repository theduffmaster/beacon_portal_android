...
public class DomainNameChecker {
	...
	private static boolean matchIpAddress(X509Certificate certificate,
		...
		try {
			Collection<List<?>> subjectAltNames = certificate
	 ...
	 * @return True iff if there is a domain match as specified by RFC2818
	 */
	private static boolean matchDns(X509Certificate certificate,
			String thisDomain) {
			...
			if (subjectAltNames != null) {
				for (List<?> altNameEntry : subjectAltNames) {
					if ((altNameEntry != null) && (2 <= altNameEntry.size())) {
						Integer altNameType = (Integer) (altNameEntry.get(0));
						if (altNameType != null
								&& altNameType.intValue() == ALT_DNS_NAME) {
							hasDns = true;
							String altName = (String) (altNameEntry.get(1));
							if (altName != null
									&& matchDns(thisDomain, altName)) {
								return true;
							}
						}
					}
	 ...
	 */
	private static boolean domainTokenMatch(String thisDomainToken,
			String thatDomainToken) {

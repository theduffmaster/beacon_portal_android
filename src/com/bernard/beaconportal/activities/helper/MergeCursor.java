/*
 * Copyright (C) 2012 The Mail Dog Walkers
...
public class MergeCursor implements Cursor {
	...
	/**
	 * The comparator that is used to decide how the individual cursors are
	 * merged.
	 */
	...
	@Override
	public int getCount() {
		// CursorLoaders seem to call getCount() a lot. So we're caching the
		// aggregated count.
		if (mCount == -1) {
			int count = 0;
			for (Cursor cursor : mCursors) {
	...
	public boolean moveToNext() {
		...
		int smallest = -1;
		for (int i = 0, len = mCursors.length; i < len; i++) {
			if (mCursors[i] == null || mCursors[i].getCount() == 0
					|| mCursors[i].isLast()) {
				continue;
			}

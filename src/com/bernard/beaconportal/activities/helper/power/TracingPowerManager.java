package com.bernard.beaconportal.activities.helper.power;
...
public class TracingPowerManager {
	...
	public static synchronized TracingPowerManager getPowerManager(
			Context context) {
		Context appContext = context.getApplicationContext();
		if (tracingPowerManager == null) {
			if (MAIL.DEBUG) {
				Log.v(MAIL.LOG_TAG, "Creating TracingPowerManager");
			}
	...
	public class TracingWakeLock {
		...
		public TracingWakeLock(int flags, String ntag) {
			...
			if (MAIL.DEBUG) {
				Log.v(MAIL.LOG_TAG, "TracingWakeLock for tag " + tag + " / id "
						+ id + ": Create");
			}
		}
		...
		public void release() {
			if (startTime != null) {
				...
				if (MAIL.DEBUG) {
					Log.v(MAIL.LOG_TAG, "TracingWakeLock for tag " + tag
							+ " / id " + id + ": releasing after "
							+ (endTime - startTime) + " ms, timeout = "
							+ timeout + " ms");
				}
			} else {
				if (MAIL.DEBUG) {
				...
				synchronized (timer) {
					...
					timerTask = new TimerTask() {
						...
						public void run() {
							if (startTime != null) {
								Long endTime = System.currentTimeMillis();
								Log.i(MAIL.LOG_TAG, "TracingWakeLock for tag "
										+ tag + " / id " + id
										+ ": has been active for "
										+ (endTime - startTime)

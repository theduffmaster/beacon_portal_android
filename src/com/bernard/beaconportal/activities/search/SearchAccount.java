 ...
 * it as an account. This is a meta-account containing all the e-mail that
 * matches the search.
 */
public class SearchAccount implements BaseAccount {
	...
	// create the all messages search ( all accounts is default when none
	// specified )
	...
	// create the unified inbox meta account ( all accounts is default when none
	// specified )
	...
	public SearchAccount(String id, LocalSearch search, String description,
	 ...
	 * 
	 * <p>
	 * This isn't really a UUID. But since we don't expose this value to other
	 * apps and we only use the account UUID as opaque string (e.g. as key in a
	 * {@code Map}) we're fine.<br>
	 * Using a constant string is necessary to identify the same search account
	 * even when the corresponding {@link SearchAccount} object has been
	 * recreated.
	 * </p>
	 */
	@Override

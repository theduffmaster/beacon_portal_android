package com.bernard.beaconportal.activities.search;
...
/**
 * This class stores search conditions. It's basically a boolean expression
 * binary tree. The output will be SQL queries ( obtained by traversing inorder
 * ).
 * 
 * TODO removing conditions from the tree TODO implement NOT as a node again
 */
public class ConditionsTreeNode implements Parcelable {
	...
	/*
	 * If mValue isn't CONDITION then mCondition contains a real condition,
	 * otherwise it's null.
	 */
	...
	/*
	 * Used for storing and retrieving the tree to/from the database. The
	 * algorithm is called "modified preorder tree traversal".
	 */
	 ...
	 * 
	 * @param cursor
	 *            Cursor pointing to the row we want to convert.
	...
	private static ConditionsTreeNode buildNodeFromRow(Cursor cursor) {
		...
		Operator tmpValue = ConditionsTreeNode.Operator.valueOf(cursor
				.getString(5));

		if (tmpValue == Operator.CONDITION) {
			condition = new SearchCondition(Searchfield.valueOf(cursor
					.getString(0)), Attribute.valueOf(cursor.getString(2)),
					cursor.getString(1));
		}
	...
	/* package */ConditionsTreeNode cloneTree() {
		if (mParent != null) {
			throw new IllegalStateException(
					"Can't call cloneTree() for a non-root node");
		}
	...
	// /////////////////////////////////////////////////////////////
	/**
	 * Adds the expression as the second argument of an AND clause to this node.
	 * 
	 * @param expr
	...
	/**
	 * Convenience method. Adds the provided condition as the second argument of
	 * an AND clause to this node.
	 * 
	 * @param condition
	...
	/**
	 * Adds the expression as the second argument of an OR clause to this node.
	 * 
	 * @param expr
	...
	/**
	 * This applies the MPTT labeling to the subtree of which this node is the
	 * root node.
	 * 
	...
	/**
	 * Returns a list of all the nodes in the subtree of which this node is the
	 * root. The list contains the nodes in a pre traversal order.
	 * 
	...
	// /////////////////////////////////////////////////////////////
	/**
	 * Adds two new ConditionTreeNodes, one for the operator and one for the new
	 * condition. The current node will end up on the same level as the one
	 * provided in the arguments, they will be siblings. Their common parent
	 * node will be one containing the operator provided in the arguments. The
	 * method will update all the required references so the tree ends up in a
	 * valid state.
	 * 
	 ...
	 *            to add.
	 * @param Operator
	 *            that will connect the new node with this one.
	 * @return New parent node, containing the operator.
	 * @throws Exception
	 *             Throws when the provided new node does not have a null
	 *             parent.
	 */
	private ConditionsTreeNode add(ConditionsTreeNode node, Operator op)
		...
		if (node.mParent != null) {
			throw new Exception(
					"Can only add new expressions from root node down.");
		}
	 ...
	 * If the provided old child node was the left one, left will be replaced
	 * with the new one. Same goes for the right one.
	 * 
	 * @param oldChild
	 ...
	 *            New child node.
	 */
	private void updateChild(ConditionsTreeNode oldChild,
			ConditionsTreeNode newChild) {
		// we can compare objects id's because this is the desired behaviour in
		// this case
		if (mLeft == oldChild) {
			mLeft = newChild;
	...
	/**
	 * Recursive function to gather all the leaves in the subtree of which this
	 * node is the root.
	 * 
	 * @param leafSet
	 *            Leafset that's being built.
	...
	private ConditionsTreeNode(Parcel in) {
		...
		mCondition = in.readParcelable(ConditionsTreeNode.class
				.getClassLoader());

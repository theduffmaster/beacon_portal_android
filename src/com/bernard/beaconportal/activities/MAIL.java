...
import com.bernard.beaconportal.activities.mail.internet.BinaryTempFileBody;
...
import com.bernard.beaconportal.activities.service.BootReceiver;
...
public class MAIL extends Application {
	/**
	 * Components that are interested in knowing when the MAIL instance is
	 * available and ready (Android invokes Application.onCreate() after other
	 * components') should implement this interface and register using
	 * {@link MAIL#registerApplicationAware(ApplicationAware)}.
	 */
	...
	public static final String LOG_TAG = "mail";

	/**
	 * Name of the {@link SharedPreferences} file used to store the last known
	 * version of the accounts' databases.
	 * 
	 * <p>
	...
	/**
	 * Key used to store the last known database version of the accounts'
	 * databases.
	 * 
	...
	/**
	 * This will be {@code true} once the initialization is complete and
	 * {@link #notifyObservers()} was called. Afterwards calls to
	 * {@link #registerApplicationAware(com.bernard.beaconportal.activities.MAIL.ApplicationAware)}
	 * will immediately call
	 * {@link com.bernard.beaconportal.activities.MAIL.ApplicationAware#initializeComponent(MAIL)}
	...
	// Environment.getExternalStorageDirectory() + "/mailmail/debug.log";
	...
	/**
	 * Can create messages containing stack traces that can be forwarded to the
	 * development team.
	 * 
	 * Feature is enabled when DEBUG == true
	 */
	public static String ERROR_FOLDER_NAME = "MAILmail-errors";

	/**
	 * A reference to the {@link SharedPreferences} used for caching the last
	 * known database version.
	 * 
	...
	public static final String LOCAL_UID_PREFIX = "MAILLOCAL:";

	public static final String REMOTE_UID_PREFIX = "MAILREMOTE:";

	public static final String IDENTITY_HEADER = "X-MAILmail-Identity";
	...
	public static class Intents {
		...
		public static class EmailReceived {
			public static final String ACTION_EMAIL_RECEIVED = "com.bernard.beaconportal.activities.mail.intent.action.EMAIL_RECEIVED";
			...
			public static final String EXTRA_SENT_DATE = "com.bernard.beaconportal.activities.intent.extra.SENT_DATE";
			public static final String EXTRA_FROM = "com.bernard.beaconportal.activities.intent.extra.FROM";
			public static final String EXTRA_TO = "com.bernard.beaconportal.activities.intent.extra.TO";
			public static final String EXTRA_CC = "com.bernard.beaconportal.activities.intent.extra.CC";
			public static final String EXTRA_BCC = "com.bernard.beaconportal.activities.intent.extra.BCC";
			public static final String EXTRA_SUBJECT = "com.bernard.beaconportal.activities.intent.extra.SUBJECT";
			public static final String EXTRA_FROM_SELF = "com.bernard.beaconportal.activities.intent.extra.FROM_SELF";
		}

		public static class Share {
			/*
			 * We don't want to use EmailReceived.EXTRA_FROM
			 * ("com.bernard.beaconportal.activities.intent.extra.FROM") because
			 * of different semantics (String array vs. string with comma
			 * separated email addresses)
			 */
			public static final String EXTRA_FROM = "com.bernard.beaconportal.activities.intent.extra.SENDER";
		}
	}
	...
	public static void setServicesEnabled(Context context) {
		int acctLength = Preferences.getPreferences(context)
				.getAvailableAccounts().size();
	...
	private static void setServicesEnabled(Context context, boolean enabled,
		...
		if (!enabled
				&& pm.getComponentEnabledSetting(new ComponentName(context,
						MailService.class)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED) {
			/*
		...
		Class<?>[] classes = { MessageCompose.class, BootReceiver.class,
				MailService.class };

		for (Class<?> clazz : classes) {

			boolean alreadyEnabled = pm
					.getComponentEnabledSetting(new ComponentName(context,
							clazz)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED;
	...
	protected void registerReceivers() {
		...
		try {
			...
			Log.i(MAIL.LOG_TAG, "Registered: unmount receiver");
		} catch (InterruptedException e) {
			Log.e(MAIL.LOG_TAG, "Unable to register unmount receiver", e);
		}

		registerReceiver(new ShutdownReceiver(), new IntentFilter(
				Intent.ACTION_SHUTDOWN));
		Log.i(MAIL.LOG_TAG, "Registered: shutdown receiver");
	}

	public static void save(SharedPreferences.Editor editor) {
		editor.putBoolean("enableDebugLogging", MAIL.DEBUG);
		editor.putBoolean("enableSensitiveLogging", MAIL.DEBUG_SENSITIVE);
		editor.putString("backgroundOperations", MAIL.backgroundOps.toString());
		...
		editor.putBoolean("messageListSenderAboveSubject",
		...
		editor.putBoolean("messageViewFixedWidthFont",
				mMessageViewFixedWidthFont);
		...
		editor.putBoolean("confirmDeleteFromNotification",
				mConfirmDeleteFromNotification);

		editor.putString("sortTypeEnum", mSortType.name());
		editor.putBoolean("sortAscending", mSortAscending.get(mSortType));

		editor.putString("notificationHideSubject",
				sNotificationHideSubject.toString());
		editor.putString("notificationQuickDelete",
				sNotificationQuickDelete.toString());

		editor.putString("attachmentdefaultpath", mAttachmentDefaultPath);
		editor.putBoolean("useBackgroundAsUnreadIndicator",
				sUseBackgroundAsUnreadIndicator);
		editor.putBoolean("threadedView", sThreadedViewEnabled);
		editor.putString("splitViewMode", sSplitViewMode.name());
		editor.putBoolean("colorizeMissingContactPictures",
				sColorizeMissingContactPictures);

		editor.putBoolean("messageViewArchiveActionVisible",
				sMessageViewArchiveActionVisible);
		editor.putBoolean("messageViewDeleteActionVisible",
				sMessageViewDeleteActionVisible);
		editor.putBoolean("messageViewMoveActionVisible",
				sMessageViewMoveActionVisible);
		editor.putBoolean("messageViewCopyActionVisible",
				sMessageViewCopyActionVisible);
		editor.putBoolean("messageViewSpamActionVisible",
				sMessageViewSpamActionVisible);
	...
	public void onCreate() {
		...
		LocalKeyStore.setKeyStoreLocation(getDir("KeyStore", MODE_PRIVATE)
				.toString());
		...
		MessagingController.getInstance(this).addListener(
				new MessagingListener() {
					private void broadcastIntent(String action,
						...
						try {
							...
							intent.putExtra(
									...
									folder);
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_SENT_DATE,
									message.getSentDate());
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_FROM,
									Address.toString(message.getFrom()));
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_TO,
									Address.toString(message
											.getRecipients(Message.RecipientType.TO)));
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_CC,
									Address.toString(message
											.getRecipients(Message.RecipientType.CC)));
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_BCC,
									Address.toString(message
											.getRecipients(Message.RecipientType.BCC)));
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_SUBJECT,
									message.getSubject());
							intent.putExtra(
									MAIL.Intents.EmailReceived.EXTRA_FROM_SELF,
									account.isAnIdentity(message.getFrom()));
							MAIL.this.sendBroadcast(intent);
							if (MAIL.DEBUG)
								Log.d(MAIL.LOG_TAG,
										"Broadcasted: action=" + action
												+ " account="
												+ account.getDescription()
												+ " folder=" + folder
												+ " message uid="
							...
							Log.w(MAIL.LOG_TAG, "Error: action=" + action
					...
					private void updateUnreadWidget() {
						try {
							UnreadWidgetProvider.updateUnreadCount(MAIL.this);
						} catch (Exception e) {
							if (MAIL.DEBUG) {
								Log.e(LOG_TAG,
										"Error while updating unread widget(s)",
										e);
							}
						}
					}
					...
					public void synchronizeMailboxRemovedMessage(
							Account account, String folder, Message message) {
						broadcastIntent(
								MAIL.Intents.EmailReceived.ACTION_EMAIL_DELETED,
								account, folder, message);
						updateUnreadWidget();
					}
					...
					@Override
					public void synchronizeMailboxNewMessage(Account account,
							String folder, Message message) {
						broadcastIntent(
								MAIL.Intents.EmailReceived.ACTION_EMAIL_RECEIVED,
								account, folder, message);
						updateUnreadWidget();
					}

					@Override
					public void folderStatusChanged(Account account,
							String folderName, int unreadMessageCount) {
	 ...
	 * 
	 * <p>
	 * If the stored version matches {@link LocalStore#DB_VERSION} we know that
	 * the databases are up to date.<br>
	 * Using {@code SharedPreferences} should be a lot faster than opening all
	 * SQLite databases to get the current database version.
	 * </p>
	 * <p>
	 * See {@link UpgradeDatabases} for a detailed explanation of the database
	 * upgrade process.
	...
	public void checkCachedDatabaseVersion() {
		sDatabaseVersionCache = getSharedPreferences(DATABASE_VERSION_CACHE,
				MODE_PRIVATE);

		int cachedVersion = sDatabaseVersionCache.getInt(
				KEY_LAST_ACCOUNT_DATABASE_VERSION, 0);

		if (cachedVersion >= LocalStore.DB_VERSION) {
			MAIL.setDatabasesUpToDate(false);
		}
	}
	...
	public static void loadPrefs(Preferences prefs) {
		...
		if (!DEBUG && sIsDebuggable && Debug.isDebuggerConnected()) {
			// If the debugger is attached, we're probably (surprise surprise)
			// debugging something.
			DEBUG = true;
			Log.i(MAIL.LOG_TAG, "Debugger attached; enabling debug logging.");
		}
		...
		mUseVolumeKeysForNavigation = sprefs.getBoolean(
				"useVolumeKeysForNavigation", false);
		mUseVolumeKeysForListNavigation = sprefs.getBoolean(
				"useVolumeKeysForListNavigation", false);
		mStartIntegratedInbox = sprefs
				.getBoolean("startIntegratedInbox", false);
		mMeasureAccounts = sprefs.getBoolean("measureAccounts", true);
		mCountSearchMessages = sprefs.getBoolean("countSearchMessages", true);
		mHideSpecialAccounts = sprefs.getBoolean("hideSpecialAccounts", false);
		mMessageListSenderAboveSubject = sprefs.getBoolean(
				"messageListSenderAboveSubject", false);
		mMessageListCheckboxes = sprefs.getBoolean("messageListCheckboxes",
				false);
		...
		mShowCorrespondentNames = sprefs.getBoolean("showCorrespondentNames",
				true);
		mShowContactName = sprefs.getBoolean("showContactName", false);
		sShowContactPicture = sprefs.getBoolean("showContactPicture", true);
		mChangeContactNameColor = sprefs.getBoolean(
				"changeRegisteredNameColor", false);
		mContactNameColor = sprefs.getInt("registeredNameColor", 0xff00008f);
		mMessageViewFixedWidthFont = sprefs.getBoolean(
				"messageViewFixedWidthFont", false);
		mMessageViewReturnToList = sprefs.getBoolean("messageViewReturnToList",
				false);
		...
		mConfirmDeleteStarred = sprefs
				.getBoolean("confirmDeleteStarred", false);
		mConfirmSpam = sprefs.getBoolean("confirmSpam", false);
		mConfirmDeleteFromNotification = sprefs.getBoolean(
				"confirmDeleteFromNotification", true);

		try {
			String value = sprefs.getString("sortTypeEnum",
					Account.DEFAULT_SORT_TYPE.name());
			mSortType = SortType.valueOf(value);
		} catch (Exception e) {
			mSortType = Account.DEFAULT_SORT_TYPE;
		}

		boolean sortAscending = sprefs.getBoolean("sortAscending",
				Account.DEFAULT_SORT_ASCENDING);
		mSortAscending.put(mSortType, sortAscending);

		String notificationHideSubject = sprefs.getString(
				"notificationHideSubject", null);
		if (notificationHideSubject == null) {
			// If the "notificationHideSubject" setting couldn't be found, the
			// app was probably
			// updated. Look for the old "keyguardPrivacy" setting and map it to
			// the new enum.
			sNotificationHideSubject = (sprefs.getBoolean("keyguardPrivacy",
					false)) ? NotificationHideSubject.WHEN_LOCKED
					: NotificationHideSubject.NEVER;
		} else {
			sNotificationHideSubject = NotificationHideSubject
					.valueOf(notificationHideSubject);
		}

		String notificationQuickDelete = sprefs.getString(
				"notificationQuickDelete", null);
		if (notificationQuickDelete != null) {
			sNotificationQuickDelete = NotificationQuickDelete
					.valueOf(notificationQuickDelete);
		}
		...
		sUseBackgroundAsUnreadIndicator = sprefs.getBoolean(
				"useBackgroundAsUnreadIndicator", true);
		...
		sColorizeMissingContactPictures = sprefs.getBoolean(
				"colorizeMissingContactPictures", true);

		sMessageViewArchiveActionVisible = sprefs.getBoolean(
				"messageViewArchiveActionVisible", false);
		sMessageViewDeleteActionVisible = sprefs.getBoolean(
				"messageViewDeleteActionVisible", true);
		sMessageViewMoveActionVisible = sprefs.getBoolean(
				"messageViewMoveActionVisible", false);
		sMessageViewCopyActionVisible = sprefs.getBoolean(
				"messageViewCopyActionVisible", false);
		sMessageViewSpamActionVisible = sprefs.getBoolean(
				"messageViewSpamActionVisible", false);

		MAIL.setMAILLanguage(sprefs.getString("language", ""));

		int themeValue = sprefs.getInt("theme", Theme.LIGHT.ordinal());
		// We used to save the resource ID of the theme. So convert that to the
		// new format if
		// necessary.
		if (themeValue == Theme.DARK.ordinal()
				|| themeValue == android.R.style.Theme) {
			MAIL.setMAILTheme(Theme.DARK);
		} else {
			MAIL.setMAILTheme(Theme.LIGHT);
		}

		themeValue = sprefs.getInt("messageViewTheme",
				Theme.USE_GLOBAL.ordinal());
		MAIL.setMAILMessageViewThemeSetting(Theme.values()[themeValue]);
		themeValue = sprefs.getInt("messageComposeTheme",
				Theme.USE_GLOBAL.ordinal());
		MAIL.setMAILComposerThemeSetting(Theme.values()[themeValue]);
		MAIL.setUseFixedMessageViewTheme(sprefs.getBoolean(
	...
	protected void notifyObservers() {
		synchronized (observers) {
			for (final ApplicationAware aware : observers) {
				if (MAIL.DEBUG) {
					Log.v(MAIL.LOG_TAG, "Initializing observer: " + aware);
				}
				try {
					aware.initializeComponent(this);
				} catch (Exception e) {
					Log.w(MAIL.LOG_TAG, "Failure when notifying " + aware, e);
				}
			}
	...
	/**
	 * Register a component to be notified when the {@link MAIL} instance is
	 * ready.
	 * 
	 * @param component
	...
	public static void registerApplicationAware(final ApplicationAware component) {
		synchronized (observers) {
			if (sInitialized) {
				component.initializeComponent(MAIL.app);
	...
	public static void setMAILLanguage(String nlanguage) {
		language = nlanguage;
	}
	 ...
	 * 
	 * <p>
	 * <strong>Important:</strong> Do not change the order of the items! The
	 * ordinal value (position) is used when saving the settings.
	 * </p>
	 */
	public enum Theme {
	...
	public static int getMAILThemeResourceId(Theme themeId) {
		return (themeId == Theme.LIGHT) ? R.style.Theme_MAIL_Light
				: R.style.Theme_MAIL_Dark;
	}

	public static int getMAILThemeResourceId() {
		return getMAILThemeResourceId(theme);
	}

	public static Theme getMAILMessageViewTheme() {
		return messageViewTheme == Theme.USE_GLOBAL ? theme : messageViewTheme;
	}

	public static Theme getMAILMessageViewThemeSetting() {
		return messageViewTheme;
	}

	public static Theme getMAILComposerTheme() {
		return composerTheme == Theme.USE_GLOBAL ? theme : composerTheme;
	}

	public static Theme getMAILComposerThemeSetting() {
		return composerTheme;
	}

	public static Theme getMAILTheme() {
		return theme;
	}

	public static void setMAILTheme(Theme ntheme) {
		if (ntheme != Theme.USE_GLOBAL) {
			theme = ntheme;
		}
	}

	public static void setMAILMessageViewThemeSetting(Theme nMessageViewTheme) {
	...
	public static void setMAILComposerThemeSetting(Theme compTheme) {
	...
	public static boolean setBackgroundOps(BACKGROUND_OPS backgroundOps) {
		BACKGROUND_OPS oldBackgroundOps = MAIL.backgroundOps;
		MAIL.backgroundOps = backgroundOps;
	...
	public static void setMessageViewReturnToList(
			boolean messageViewReturnToList) {
	...
	public static void setUseGalleryBugWorkaround(
		...
		MAIL.useGalleryBugWorkaround = useGalleryBugWorkaround;
	}
	...
	public static void setNotificationHideSubject(
			final NotificationHideSubject mode) {
	...
	public static void setAttachmentDefaultPath(String attachmentDefaultPath) {
		MAIL.mAttachmentDefaultPath = attachmentDefaultPath;
	}
	...
	public static synchronized void setSortAscending(SortType sortType,
	...
	public static synchronized void setUseBackgroundAsUnreadIndicator(
			boolean enabled) {
	...
	/**
	 * Check if we already know whether all databases are using the current
	 * database schema.
	 * 
	 * <p>
	 * This method is only used for optimizations. If it returns {@code true} we
	 * can be certain that getting a {@link LocalStore} instance won't trigger a
	 * schema upgrade.
	 * </p>
	 * 
	 * @return {@code true}, if we know that all databases are using the current
	 *         database schema. {@code false}, otherwise.
	 */
	...
	/**
	 * Remember that all account databases are using the most recent database
	 * schema.
	 * 
	 * @param save

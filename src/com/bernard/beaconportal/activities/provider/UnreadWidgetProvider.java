...
public class UnreadWidgetProvider extends AppWidgetProvider {
	...
	public static void updateUnreadCount(Context context) {
		...
		ComponentName thisWidget = new ComponentName(appContext,
	...
	public static void updateWidget(Context context,
		...
		try {
			...
			if (SearchAccount.UNIFIED_INBOX.equals(accountUuid)) {
				searchAccount = SearchAccount
						.createUnifiedInboxAccount(context);
			...
			if (searchAccount != null) {
				...
				Account realAccount = Preferences.getPreferences(context)
				...
				if (realAccount != null) {
					...
					if (MAIL.FOLDER_NONE.equals(realAccount
						...
						search.addAllowedFolder(realAccount
								.getAutoExpandFolderName());
		...
		if (unreadCount <= 0) {
			...
			String displayCount = (unreadCount <= MAX_COUNT) ? String
					.valueOf(unreadCount) : String.valueOf(MAX_COUNT) + "+";
		...
		if (clickIntent == null) {
			// If the widget configuration couldn't be loaded we open the
			// configuration
		...
		PendingIntent pendingIntent = PendingIntent.getActivity(context,
				appWidgetId, clickIntent, 0);

		remoteViews.setOnClickPendingIntent(R.id.unread_widget_layout,
				pendingIntent);
	...
	public void onDeleted(Context context, int[] appWidgetIds) {
		for (int appWidgetId : appWidgetIds) {
			UnreadWidgetConfiguration.deleteWidgetConfiguration(context,
					appWidgetId);
		}
	}
}

...
public class MessageProvider extends ContentProvider {
	...
	public static class IdExtractor implements
			FieldExtractor<MessageInfoHolder, Long> {
		@Override
	...
	public static class SubjectExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
	...
	public static class SendDateExtractor implements
			FieldExtractor<MessageInfoHolder, Long> {
		@Override
		public Long getField(final MessageInfoHolder source) {
	...
	public static class PreviewExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
		public String getField(final MessageInfoHolder source) {
	...
	public static class UriExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
		public String getField(final MessageInfoHolder source) {
			return source.uri;
	...
	public static class DeleteUriExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
		public String getField(final MessageInfoHolder source) {
	...
	public static class SenderExtractor implements
			FieldExtractor<MessageInfoHolder, CharSequence> {
		@Override
	...
	public static class SenderAddressExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
		public String getField(final MessageInfoHolder source) {
	...
	public static class AccountExtractor implements
			FieldExtractor<MessageInfoHolder, String> {
		@Override
		public String getField(final MessageInfoHolder source) {
	...
	public static class AccountColorExtractor implements
			FieldExtractor<MessageInfoHolder, Integer> {
		@Override
	...
	public static class AccountNumberExtractor implements
			FieldExtractor<MessageInfoHolder, Integer> {
		@Override
		public Integer getField(final MessageInfoHolder source) {
	...
	public static class HasAttachmentsExtractor implements
			FieldExtractor<MessageInfoHolder, Boolean> {
		@Override
	...
	public static class HasStarExtractor implements
			FieldExtractor<MessageInfoHolder, Boolean> {
		@Override
		public Boolean getField(final MessageInfoHolder source) {
	...
	public static class UnreadExtractor implements
			FieldExtractor<MessageInfoHolder, Boolean> {
		@Override
		public Boolean getField(final MessageInfoHolder source) {
	...
	/**
	 * @deprecated having an incremental value has no real interest, implemented
	 *             for compatibility only
	 */
	@Deprecated
	// TODO remove
	public static class IncrementExtractor implements
			FieldExtractor<MessageInfoHolder, Integer> {
		private int count = 0;
	...
	protected class MessagesQueryHandler implements QueryHandler {
		 ...
		 */
		protected MatrixCursor getMessages(final String[] projection)
				throws InterruptedException {
			...
			final String[] actualProjection = extractors.keySet().toArray(
			...
			for (final MessageInfoHolder holder : holders) {
				...
				int i = 0;
				for (final FieldExtractor<MessageInfoHolder, ?> extractor : extractors
						.values()) {
	...
	protected class AccountsQueryHandler implements QueryHandler {
		...
		public Cursor getAllAccounts(String[] projection) {
			// Default projection
			if (projection == null) {
				projection = new String[] { FIELD_ACCOUNT_NUMBER,
						FIELD_ACCOUNT_NAME };
			}

			MatrixCursor ret = new MatrixCursor(projection);

			for (Account account : Preferences.getPreferences(getContext())
					.getAccounts()) {
	...
	protected class UnreadQueryHandler implements QueryHandler {
		...
		public Cursor query(final Uri uri, String[] projection,
			...
			/*
			 * This method below calls Account.getStats() which uses
			 * EmailProvider to do its work. For this to work we need to clear
			 * the calling identity. Otherwise accessing EmailProvider will fail
			 * because it's not exported so third-party apps can't access it
			 * directly.
			 */
	...
	protected class ThrottlingQueryHandler implements QueryHandler {
		...
		public Cursor query(Uri uri, String[] projection, String selection,
			...
			Cursor cursor = null;
			try {
				cursor = mDelegate.query(uri, projection, selection,
			...
			final MonitoredCursor wrapped = new MonitoredCursor(
					(CrossProcessCursor) cursor, mSemaphore);

			/* use a weak reference not to actively prevent garbage collection */
			final WeakReference<MonitoredCursor> weakReference = new WeakReference<MonitoredCursor>(
					wrapped);
	 ...
	 */
	protected class MesssageInfoHolderRetrieverListener extends
			MessagingListener {
		 ...
		 */
		public MesssageInfoHolderRetrieverListener(
		...
		public void listLocalMessagesAddMessages(final Account account,
			...
			for (final Message message : messages) {
				...
				helper.populate(messageInfoHolder, message,
						new FolderInfoHolder(context, messageFolder,
	...
	public static final String AUTHORITY = "com.bernard.beaconportal.activities.messageprovider";

	public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY);

	private static final String[] DEFAULT_MESSAGE_PROJECTION = new String[] {
			MessageColumns._ID, MessageColumns.SEND_DATE,
			MessageColumns.SENDER, MessageColumns.SUBJECT,
			MessageColumns.PREVIEW, MessageColumns.ACCOUNT, MessageColumns.URI,
			MessageColumns.DELETE_URI, MessageColumns.SENDER_ADDRESS };
	...
	/* package */
	ScheduledExecutorService mScheduledPool = Executors
	...
	public boolean onCreate() {
		...
		registerQueryHandler(new ThrottlingQueryHandler(
				new AccountsQueryHandler()));
		registerQueryHandler(new ThrottlingQueryHandler(
				new MessagesQueryHandler()));
		registerQueryHandler(new ThrottlingQueryHandler(
	...
	public static class ReverseDateComparator implements
			Comparator<MessageInfoHolder> {
		@Override

...
import com.bernard.beaconportal.activities.preferences.GlobalSettings;
...
public class FontSizes {
	 ...
	 */
	public static final int FONT_DEFAULT = -1; // Don't force-reset the size of
												// this setting
	...
	/**
	 * Font size of the message content in the message view activity, as percent
	 * from default size.
	 */
	...
	public void save(SharedPreferences.Editor editor) {
		...
		editor.putInt(MESSAGE_VIEW_ADDITIONAL_HEADERS,
		...
		editor.putInt(MESSAGE_VIEW_CONTENT_PERCENT,
	 ...
	 * 
	 * @param prefs
	 *            Used to load the font size settings.
	 */
	public void load(SharedPreferences prefs) {
		accountName = prefs.getInt(ACCOUNT_NAME, accountName);
		accountDescription = prefs.getInt(ACCOUNT_DESCRIPTION,
				accountDescription);

		folderName = prefs.getInt(FOLDER_NAME, folderName);
		folderStatus = prefs.getInt(FOLDER_STATUS, folderStatus);

		messageListSubject = prefs.getInt(MESSAGE_LIST_SUBJECT,
				messageListSubject);
		messageListSender = prefs
				.getInt(MESSAGE_LIST_SENDER, messageListSender);
		messageListDate = prefs.getInt(MESSAGE_LIST_DATE, messageListDate);
		messageListPreview = prefs.getInt(MESSAGE_LIST_PREVIEW,
				messageListPreview);

		messageViewSender = prefs
				.getInt(MESSAGE_VIEW_SENDER, messageViewSender);
		messageViewTo = prefs.getInt(MESSAGE_VIEW_TO, messageViewTo);
		messageViewCC = prefs.getInt(MESSAGE_VIEW_CC, messageViewCC);
		messageViewAdditionalHeaders = prefs.getInt(
				MESSAGE_VIEW_ADDITIONAL_HEADERS, messageViewAdditionalHeaders);
		messageViewSubject = prefs.getInt(MESSAGE_VIEW_SUBJECT,
				messageViewSubject);
		messageViewDate = prefs.getInt(MESSAGE_VIEW_DATE, messageViewDate);

		loadMessageViewContentPercent(prefs);

		messageComposeInput = prefs.getInt(MESSAGE_COMPOSE_INPUT,
				messageComposeInput);
	}
	...
	private void loadMessageViewContentPercent(SharedPreferences prefs) {
		...
		if (!prefs.contains(MESSAGE_VIEW_CONTENT_PERCENT)) {
			...
			fallbackValue = GlobalSettings.SettingsUpgraderV31
					.convertFromOldSize(oldValue);
		}

		setMessageViewContentAsPercent(prefs.getInt(
				MESSAGE_VIEW_CONTENT_PERCENT, fallbackValue));
	}
	...
	// This, arguably, should live somewhere in a view class, but since we call
	// it from activities, fragments

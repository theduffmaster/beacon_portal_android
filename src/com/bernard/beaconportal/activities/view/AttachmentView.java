...
public class AttachmentView extends FrameLayout implements OnClickListener,
	 ...
	 *             In case of an error
	 */
	public boolean populateFromPart(Part inputPart, Message message,
			Account account, MessagingController controller,
		...
		String contentDisposition = MimeUtility.unfoldAndDecode(part
				.getDisposition());
		...
		String sizeParam = MimeUtility.getHeaderParameter(contentDisposition,
				"size");
		if (sizeParam != null) {
			try {
		...
		contentType = MimeUtility.getMimeTypeForViewing(part.getMimeType(),
				name);
		...
		new AsyncTask<Void, Void, Bitmap>() {
			...
			protected void onPostExecute(Bitmap previewIcon) {
				if (previewIcon != null) {
					...
					attachmentIcon
							.setImageResource(R.drawable.attached_image_placeholder);
				}
			}
		}.execute();
	...
	private void onViewButtonClicked() {
		if (mMessage != null) {
			mController.loadAttachment(mAccount, mMessage, part, new Object[] {
	...
	public void showFile() {
		Uri uri = AttachmentProvider.getAttachmentUriForViewing(mAccount,
				part.getAttachmentId());

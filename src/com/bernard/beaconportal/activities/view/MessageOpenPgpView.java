...
public class MessageOpenPgpView extends LinearLayout {
	...
	public void updateLayout(Account account, String decryptedData,
		...
		if (decryptedData != null && signatureResult == null) {
			...
			MessageOpenPgpView.this.setBackgroundColor(mFragment.getResources()
					.getColor(R.color.openpgp_blue));
			...
			case OpenPgpSignatureResult.SIGNATURE_ERROR:
				...
				MessageOpenPgpView.this.setBackgroundColor(mFragment
						.getResources().getColor(R.color.openpgp_red));

				mGetKeyButton.setVisibility(View.GONE);
				mSignatureStatusImage
			...
			case OpenPgpSignatureResult.SIGNATURE_SUCCESS_CERTIFIED:
				...
				MessageOpenPgpView.this.setBackgroundColor(mFragment
						.getResources().getColor(R.color.openpgp_green));
			...
			case OpenPgpSignatureResult.SIGNATURE_UNKNOWN_PUB_KEY:
				...
				MessageOpenPgpView.this.setBackgroundColor(mFragment
						.getResources().getColor(R.color.openpgp_orange));
		...
		if (helper.isEncrypted(message) || helper.isSigned(message)) {
			...
			try {
				...
				if (pgp != null) {
					Toast.makeText(mContext, R.string.pgp_mime_unsupported,
	...
	private void decryptAndVerify(final Message message) {
		...
		MessageOpenPgpView.this.setBackgroundColor(mFragment.getResources()
				.getColor(R.color.openpgp_orange));
	...
	private void decryptVerify(Intent intent) {
		...
		Identity identity = IdentityHelper.getRecipientIdentityFromMessage(
				mAccount, mMessage);
		...
		DecryptVerifyCallback callback = new DecryptVerifyCallback(os,
				REQUEST_CODE_DECRYPT_VERIFY);
	...
	public boolean handleOnActivityResult(int requestCode, int resultCode,
		...
		if (resultCode == Activity.RESULT_OK
				&& requestCode == REQUEST_CODE_DECRYPT_VERIFY) {
			/*
	...
	protected void onAttachedToWindow() {
		...
		if (mOpenPgpProvider != null) {
			mOpenPgpServiceConnection = new OpenPgpServiceConnection(
					mFragment.getActivity(), mOpenPgpProvider);
			mOpenPgpServiceConnection.bindToService();
		}
	}

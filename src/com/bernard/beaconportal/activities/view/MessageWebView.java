...
public class MessageWebView extends RigidWebView {
	...
	/**
	 * Check whether the single column layout algorithm can be used on this
	 * version of Android.
	 * 
	 * <p>
	 * Single column layout was broken on Android < 2.2 (see <a
	 * href="http://code.google.com/p/android/issues/detail?id=5024">issue
	 * 5024</a>).
	 * </p>
	 * 
	 * <p>
	 * Android versions >= 3.0 have problems with unclickable links when single
	...
	/**
	 * Configure a web view to load or not load network data. A <b>true</b>
	 * setting here means that network data will be blocked.
	 * 
	 * @param shouldBlockNetworkData
	 *            True if network data should be blocked, false to allow network
	 *            data.
	 */
	public void blockNetworkData(final boolean shouldBlockNetworkData) {
		 ...
		 * Block network loads.
		 * 
		 * Images with content: URIs will not be blocked, nor will network
		 * images that are already in the WebView cache.
		 */
		getSettings().setBlockNetworkLoads(shouldBlockNetworkData);
	}

	/**
	 * Configure a {@link android.webkit.WebView} to display a Message. This
	 * method takes into account a user's preferences when configuring the view.
	 * This message is used to view a message and to display a message being
	 * replied to.
	 */
	...
	/**
	 * Disable on-screen zoom controls on devices that support zooming via
	 * pinch-to-zoom.
	 */
	@TargetApi(11)
	 ...
	 * 
	 * <p>
	 * Before loading, the text is wrapped in an HTML header and footer so that
	 * it displays properly.
	 * </p>
	 * 
	 * @param text
	 *            The message body to display. Assumed to be MIME type
	 *            text/html.
	 */
	...
	/*
	 * Emulate the shift key being pressed to trigger the text selection mode of
	 * a WebView.
	 */

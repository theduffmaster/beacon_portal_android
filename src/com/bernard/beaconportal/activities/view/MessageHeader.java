...
public class MessageHeader extends LinearLayout implements OnClickListener {
	...
	/**
	 * Pair class is only available since API Level 5, so we need this helper
	 * class unfortunately
	 */
	...
	protected void onFinishInflate() {
		...
		mFontSizes.setViewTextSize(mAdditionalHeadersView,
				mFontSizes.getMessageViewAdditionalHeaders());

		mFontSizes
				.setViewTextSize(mFromView, mFontSizes.getMessageViewSender());
	...
	public boolean additionalHeadersVisible() {
		return (mAdditionalHeadersView != null && mAdditionalHeadersView
				.getVisibility() == View.VISIBLE);
	}

	/**
	 * Clear the text field for the additional headers display if they are not
	 * shown, to save UI resources.
	 */
	 ...
	 * {@link #onShowAdditionalHeaders()} (when switching between messages).
	 */
	private void showAdditionalHeaders() {
		Integer messageToShow = null;
		try {
			// Retrieve additional headers
			boolean allHeadersDownloaded = mMessage
					.isSet(Flag.X_GOT_ALL_HEADERS);
			...
			if (!allHeadersDownloaded) {
				 ...
				 * 
				 * NOTE: This is only a temporary solution... in fact, the
				 * system should download headers on-demand when they have not
				 * been saved in their entirety initially.
				 */
				messageToShow = R.string.message_additional_headers_not_downloaded;
			} else if (additionalHeaders.isEmpty()) {
				// All headers have been downloaded, but there are no additional
				// headers.
		...
		if (messageToShow != null) {
			Toast toast = Toast.makeText(mContext, messageToShow,
					Toast.LENGTH_LONG);
	...
	public void populate(final Message message, final Account account)
		...
		final CharSequence to = Address.toFriendly(
				message.getRecipients(Message.RecipientType.TO), contacts);
		final CharSequence cc = Address.toFriendly(
				message.getRecipients(Message.RecipientType.CC), contacts);
		...
		/*
		 * Only reset visibility of the subject if populate() was called because
		 * a new message is shown. If it is the same, do not force the subject
		 * visible, because this breaks the MessageTitleView in the action bar,
		 * which may hide our subject if it fits in the action bar but is only
		 * called when a new message is shown or the device is rotated.
		 */
		...
		if (MAIL.showContactPicture()) {
			if (counterpartyAddress != null) {
				...
				mContactsPictureLoader.loadContactPicture(counterpartyAddress,
						mContactBadge);
			} else {
		...
		mAnsweredIcon.setVisibility(message.isSet(Flag.ANSWERED) ? View.VISIBLE
				: View.GONE);
		mForwardedIcon
	...
	/**
	 * Expand or collapse a TextView by removing or adding the 2 lines
	 * limitation
	 */
	 ...
	 *            List of header entries. Each entry consists of a header name
	 *            and a header value. Header names may appear multiple times.
	 *            <p/>
	 *            This method is always called from within the UI thread by
	 *            {@link #showAdditionalHeaders()}.
	 */
	private void populateAdditionalHeadersView(

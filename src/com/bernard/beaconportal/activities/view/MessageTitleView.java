...
/**
 * This {@link TextView} is used in the custom view of the
 * {@link com.bernard.beaconportal.activities.activity.MessageList} action bar.
 * It will hide the subject line in {@link MessageHeader} if the subject fits
 * completely into the action bar's title view.
 */
public class MessageTitleView extends TextView {
	...
	/**
	 * Check to see if we need to hide the subject line in {@link MessageHeader}
	 * or not.
	 */
	@Override
	public void onDraw(Canvas canvas) {
		/*
		 * Android does not support ellipsize in combination with maxlines for
		 * TextViews. To work around that, check for ourselves whether the text
		 * is longer than MAX_LINES, and ellipsize manually.
		 */
		if (mNeedEllipsizeCheck) {
			if (getLayout() != null && mHeader != null) {
				if (getLayout().getLineCount() > MAX_LINES) {
					...
					setText(getText().subSequence(0, lineEndIndex - 2)
							+ ELLIPSIS);
				} else {
